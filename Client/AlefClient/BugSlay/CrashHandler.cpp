...
BOOL __stdcall SetCrashHandlerFilter ( PFNCHFILTFN pFn )
{
    ...
    else
    {
        ...
        if ( ( BOOL ) TRUE == IsBadCodePtr ( (FARPROC)pFn ) )
        {
            return ( FALSE ) ;
        }
        g_pfnCallBack = pFn ;
...
int __stdcall GetLimitModulesArray ( HMODULE * pahMod , UINT uiSize )
{
    ...
    __try
    {
        ...
        if ( ( BOOL ) TRUE == IsBadWritePtr ( pahMod ,
                                     uiSize * sizeof ( HMODULE ) ) )
        {
            iRet = GLMA_BADPARAM ;
            __leave ;
...
LONG __stdcall CrashHandlerExceptionFilter (EXCEPTION_POINTERS* pExPtrs)
{
	...
	INT16	nSize;
	PVOID	pvPacket = g_pEngine->MakeSPClientExit(&nSize);

	if (pvPacket)
	{
		g_pEngine->SendPacket(pvPacket, nSize);

		g_pEngine->FreeSystemPacket(pvPacket);
	}

	g_pEngine->m_SocketManager.DisconnectAll();
...
LPCTSTR __stdcall GetFaultReason ( EXCEPTION_POINTERS * pExPtrs , int iVersion )
{
    ASSERT ( FALSE == IsBadReadPtr ( pExPtrs ,
                                     sizeof ( EXCEPTION_POINTERS ) ) ) ;
    if ( ( BOOL ) TRUE == IsBadReadPtr ( pExPtrs ,
                                sizeof ( EXCEPTION_POINTERS ) ) )
    {
    ...
    __try
    {
		...
		// 기본적인 크래시 정보를 첫 라인에 남겨서 프로그램에서 읽기 좋게함.
		// 크래시프로세스,크래시모듈,크래스펑션 
		{
			// 프로세스.
			...
			// 마고자 (2004-07-27 오후 2:40:21) : 버젼 정보 남김.
			iCurr += wsprintf ( g_szBuff + iCurr    ,
								_T ( " %d\n"		),
								iVersion			) ;
		}
                                  ...
                                  UINT                 uiSize   )
{
    ...
    if ( ( BOOL ) TRUE == IsBadWritePtr ( szBuff , uiSize ) )
    {
        return ( FALSE ) ;
    }
