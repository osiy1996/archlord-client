...
void AgcmTeleportTarget::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_USE_DRAGONSCION, m_bUseTypeDragonScion);
}
...
void	AgcmTeleportTarget::OnOK() 
{
	...
	AgpdTeleportGroup *	pcsGroup = NULL;

	INT32 nTargetGroupCount = 0;
	pcsGroup = m_pcsAgpmEventTeleport->GetSequenceTargetGroup( m_pcsPoint, &nTargetGroupCount );
	while( pcsGroup )
	{
		m_pcsAgpmEventTeleport->RemoveTargetGroupFromPoint( pcsGroup, m_pcsPoint );
		pcsGroup = m_pcsAgpmEventTeleport->GetSequenceTargetGroup( m_pcsPoint, &nTargetGroupCount );
	}
	...
	if( m_bUseTypeDragonScion )	m_pcsPoint->SetUseType( AgpdTeleportPoint::eTeleportUseDragonScion );
...
BOOL	AgcmTeleportTarget::InitDialogData()
{
	...
	m_bUseTypeDragonScion	= m_pcsPoint->IsUseDragonScion();
