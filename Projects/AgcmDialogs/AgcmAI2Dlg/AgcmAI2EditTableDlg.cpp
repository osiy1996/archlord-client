...
void AgcmAI2EditTableDlg::FillControlByItem( AgpdAI2UseItem *pcsItem )
{
	...
	pcsAgpaItemTemplate = &m_pcsAgpmItem->csTemplateAdmin;

	AgpaItemTemplate::iterator	Iter	=	pcsAgpaItemTemplate->find( pcsItem->m_lItemID );
	if( Iter == pcsAgpaItemTemplate->end() )
		pcsItemTemplate	=	NULL;
	else
	...
	{
		AgpaItemTemplate::iterator	Iter		=	pcsAgpaItemTemplate->begin();
		for(  ; Iter != pcsAgpaItemTemplate->end() ; ++Iter )
		{
			AgpdItemTemplate*		pTemplate	=	Iter->second;
			if( !pTemplate )
				continue;

			if( pTemplate->m_lID == pcsItem->m_lItemID )
			{
				pcsItemTemplate	=	pTemplate;
				break;
			}
		}
...
void AgcmAI2EditTableDlg::FillComboItem()
{
	...
	for ( ; Iter != pcsAgpaItemTemplate->end() ; ++Iter )
	{
		...
		iItemData	=	m_cComboBox.AddString( pTemplate->m_szName );
		if( iItemData != CB_ERR )
			m_cComboBox.SetItemData( iItemData , pTemplate->m_lID );
