...
void ServerEngine::OpenIPInfo( bool &bUseIPBlock, bool &bOpenPCRoom )
{
	...
	{
		printf( "\n[Start] 시작입니다.\n" );
		printf( "[Step-1] PC방 IP를 여시겠습니까?(y/n)" );
		scanf( "%c", &cData );
		getchar();

		if( cData == 'y' || cData == 'Y' )
		{
			printf( "[Step-2] 정말로 PC방 IP를 여시겠습니까?(y/n)" );
			scanf( "%c", &cData );
			getchar();

			if( cData == 'y' || cData == 'Y' )
			{
				printf( "[Step-3] **지정된 PC방을 통해 접속하면 테스터가 아니어도 접속 가능합니다.\n" );
				bOpenPCRoom = true;
				break;
			}
		}
		...
		{
			printf( "[Step-2] PC방사용자 IP는 추가되지 않습니다.(y/n)" );
			scanf( "%c", &cData );
			getchar();

			if( cData == 'y' || cData == 'Y' )
			{
				printf( "[Step-3] --PC방을 통한 접속이 불가능합니다.--\n" );
				bOpenPCRoom = false;
				break;
			}
		}
