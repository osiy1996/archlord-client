...
BOOL AgcmUIStatus::CBReceiveQuickBeltInfo(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	for (int nLayer	= 0; nLayer < AGPMUISTATUS_MAX_QUICKBELT_LAYER; ++nLayer)
	{
		...
		for (int i = nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN; 
			i < nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + 
				AGPMUISTATUS_HALF_QUICKBELT_COLUMN; 
			++i) {
			BOOL bIsAdded = FALSE;

			if (pcsAttachData->m_acsQBeltItem[i].m_eType != APBASE_TYPE_NONE &&
				pcsAttachData->m_acsQBeltItem[i].m_lID != 0) {
				ApBase * pcsBase = pThis->m_pcsAgpmSkill->GetBase(
					pcsAttachData->m_acsQBeltItem[i].m_eType, 
					pcsAttachData->m_acsQBeltItem[i].m_lID);

				if (pcsBase) {
					switch (pcsBase->m_eType) {
					case APBASE_TYPE_ITEM:
						bIsAdded = pThis->m_pcsAgcmUIMain->AddShortcut(
							&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[nLayer * 2], 
							((AgpdItem *) pcsBase)->m_pcsGridItem, 
							0, 0, nColumn, FALSE);
						break;
					case APBASE_TYPE_SKILL:
						bIsAdded = pThis->m_pcsAgcmUIMain->AddShortcut(
							&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[nLayer * 2], 
							((AgpdSkill *) pcsBase)->m_pcsGridItem, 
							0, 0, nColumn, FALSE);
						break;
					}
				}
			...
			if (!bIsAdded)
				pThis->m_pcsAgcmUIMain->RemoveShortcut(
					&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[nLayer * 2], 
					0, 0, nColumn);
			++nColumn;
		}
		...
		for (int i = nLayer * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + 
				AGPMUISTATUS_HALF_QUICKBELT_COLUMN; 
			i < (nLayer + 1) * AGPMUISTATUS_MAX_QUICKBELT_COLUMN; 
			++i) {
			BOOL bIsAdded = FALSE;
			...
			if (pcsAttachData->m_acsQBeltItem[i].m_eType != APBASE_TYPE_NONE &&
				...
				if (pcsBase) {
					...
					case APBASE_TYPE_ITEM:
						bIsAdded = pThis->m_pcsAgcmUIMain->AddShortcut(
							&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[nLayer * 2 + 1], 
							((AgpdItem *)pcsBase)->m_pcsGridItem, 
							0, 0, nColumn, FALSE);
						break;
					case APBASE_TYPE_SKILL:
...
BOOL AgcmUIStatus::CBAddShortcut(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	for (size_t i = 0; i < AGCMUIMAIN_MAX_QUICKBELT; i++) {
		if (&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[i * 2] == pcsGrid) {
			...
			pThis->SendPacketUpdateQBeltItem(pcsSelfCharacter, i * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + lColumn, pcsBaseItem);
		}
		else if (&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[i * 2 + 1] == pcsGrid) {
			ApBase			*pcsBaseItem		= NULL;
			...
			pThis->SendPacketUpdateQBeltItem(pcsSelfCharacter, i * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + lColumn + AGPMUISTATUS_HALF_QUICKBELT_COLUMN, pcsBaseItem);
		}
	}
	return TRUE;
}
...
BOOL AgcmUIStatus::CBRemoveShortcut(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (bIsCleared) {
		for (size_t i = 0; i < AGCMUIMAIN_MAX_QUICKBELT; i++) {
			if (&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[i * 2] == pcsGrid)
				pThis->SendPacketUpdateQBeltItem(pcsSelfCharacter, i * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + lColumn, NULL);
			else if (&pThis->m_pcsAgcmUIMain->m_quickBeltGrid[i * 2 + 1] == pcsGrid)
				pThis->SendPacketUpdateQBeltItem(pcsSelfCharacter, i * AGPMUISTATUS_MAX_QUICKBELT_COLUMN + lColumn + AGPMUISTATUS_HALF_QUICKBELT_COLUMN, NULL);
		}
	}
	return TRUE;
}
