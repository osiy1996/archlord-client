...
//		((AcUIScroll *) pThis->m_pcsScrollControl->m_pcsBase)->SetScrollValue(1.0f);
	...
	if (pstChatData && pstChatData->eChatType == AGPDCHATTING_TYPE_WHOLE_WORLD)
	{
		...
		pThis->m_pcsAgcmUIManager2->Notice(szMessage, -1, NULL, -1, -1, 0.2f);
		...
		else if(!strcmp(command, "/postfx"))
		{
			...
			if(arg.GetArgCount() >= 2)
			{
				...
				if(arg.GetParam(1)[0] == '-')
				{
					...
					{
						if(pPostFX->m_listTechnic.size() == 0)
						{
							m_pcsAgcmChatting2->AddSystemMessage("no techniques are available.");
						}
						else
						{
							for( INT i = 0 ;  i  < pPostFX->GetCount(); ++i)
							{
								Technique*	pTech	=	pPostFX->GetInstance(i);

								m_pcsAgcmChatting2->AddSystemMessage( pTech->m_name );
							}

							pPostFX->GetInstance( 0 );
						...
						// print the current post fx pipeline.
					{
						...
						{
							for(int i=0; i< ( int ) pPostFX->m_listTechnic.size(); ++i)
							{
								if(i==0)
								{
									//strcpy(buf, pPostFX->m_listTechnic[pPostFX->m_listTechnic[i]->m_nFxIndex]->m_name);
								}
								else
								{
									//strcat(buf, "-");
									//strcat(buf, pPostFX->m_listTechnic[pPostFX->m_listTechnic[i]->m_nFxIndex]->m_name);
								}
							}
				...
				else
				{
					...
					m_pcsAgcmChatting2->AddSystemMessage(buf);
					//if(pPostFX->CreateFX(arg.GetParam(1))==FALSE)
					//{
			...
			else
			{
				...
				m_pcsAgcmChatting2->AddSystemMessage("refresh post fx.");
				pPostFX->PostFX_OFF();
				pPostFX->PostFX_ON();
			}
		}
