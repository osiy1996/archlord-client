...
AgcmEventEffect::AgcmEventEffect()
{
	...
	m_bDisplayLensNormalDmg = TRUE;
	m_bDisplayLensSkillDmg = TRUE;
	m_bDisplayLensSkillEff = TRUE;
...
BOOL AgcmEventEffect::EventCharActionResult(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	case AGPDCHAR_ACTION_TYPE_ATTACK:
		...
		if (DEF_FLAG_CHK(pcsActionData->ulAdditionalEffect, 
			AGPDCHAR_ADDITIONAL_EFFECT_LENS_STONE) && 
			pcsSelfCharacter->m_lID == pcsAgpdActor->m_lID &&
			pcsThis->m_bDisplayLensNormalDmg)
			pcsThis->SetCommonCharacterEffect(pcsAgpdTarget, 
	...
	case AGPDCHAR_ACTION_TYPE_SKILL:
		if (DEF_FLAG_CHK(pcsActionData->ulAdditionalEffect, 
			AGPDCHAR_ADDITIONAL_EFFECT_LENS_STONE) && 
			pcsSelfCharacter->m_lID == pcsAgpdActor->m_lID &&
			pcsThis->m_bDisplayLensSkillDmg)
			pcsThis->SetCommonCharacterEffect(pcsAgpdTarget, 
				E_COMMON_CHAR_TYPE_LENS_STONE);
		break;
	}
...
BOOL AgcmEventEffect::EventCharAdditionalEffect(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (pThis->m_bDisplayLensSkillEff)
...
void
AgcmEventEffect::DisplayLensNormalDamage(BOOL bValue)
{
	m_bDisplayLensNormalDmg = bValue;
}

void
AgcmEventEffect::DisplayLensSkillDamage(BOOL bValue)
{
	m_bDisplayLensSkillDmg = bValue;
}

void
AgcmEventEffect::DisplayLensSkillEffect(BOOL bValue)
{
	m_bDisplayLensSkillEff = bValue;
}
