...
#include <AgcmMinimap/AcUIWorldmap.h>
...
void AcUIWorldmap::OnWindowRender	()
{
...

	{
		...
		int half_w = m_pAgcmMinimap->m_pcsAgcmFont->GetTextExtent(0, point.empty() ? "" : (char*)point.c_str(), -1) >> 1;

		m_pAgcmMinimap->m_pcsAgcmFont->DrawTextIM2D(
			( FLOAT ) nPopupX - half_w - 1, ( FLOAT ) nPopupY + 1,
