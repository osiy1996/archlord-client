...
// 		m_pcsAgcmSound->Play3DSound( "Sound\\Effect\\JY0002.wav", 1, 1.0f, TRUE, pcsAgcdCharacter->m_pClump, 1.0f, TRUE );
...
// 		m_pcsAgcmSound->Play3DSound( "Sound\\Effect\\JY0000.wav", 1, 1.0f, TRUE, pcsAgcdCharacter->m_pClump, 1.0f, TRUE );
...
// 		m_pcsAgcmSound->Play3DSound( "Sound\\Effect\\JY0001.wav", 1, 1.0f, TRUE, pcsAgcdCharacter->m_pClump, 1.0f, TRUE );
...
RwTexture * AgcmTextBoardMng::GetTeamFlagTexture(UINT64 uiState)
{
	if (uiState & AGPDCHAR_SPECIAL_STATUS_FLAG_BLUE)
		return m_pcsAgcmUIControl->m_pTextureFlagBlue;
	if (uiState & AGPDCHAR_SPECIAL_STATUS_FLAG_RED)
		return m_pcsAgcmUIControl->m_pTextureFlagRed;
	return NULL;
}

RwTexture * AgcmTextBoardMng::GetBestOfClassTexture(AgpdCharacter * pCharacter)
{
	AuRace stRace = m_pcsAgpmCharacter->GetCharacterRace(pCharacter);
	INT16 nRace = stRace.detail.nRace;
	INT16 nClass = stRace.detail.nClass;
	if (nRace < 0 || nRace >= AURACE_TYPE_MAX ||
		nClass < 0 || nClass >= AUCHARCLASS_TYPE_MAX)
		return NULL;
	if (pCharacter->m_ulSpecialStatus & AGPDCHAR_SPECIAL_STATUS_BEST_OF_CLASS)
		return m_pcsAgcmUIControl->m_pTextureBestOfClass[nRace][nClass];
	return NULL;
}
