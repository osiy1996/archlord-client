...
#include <rpcollis.h>
#include <rtpick.h>
#include <AgcmTargeting/AgcSenderWindow.h>
#include <string>
...
// data
	...
	class AgcmPrivateTrade*		m_pAgcmPrivateTrade		;
	class AgcmRender*			m_pAgcmRender			;
	class AgcmParty*			m_pAgcmParty			;
	class AgpmFactors*			m_pAgpmFactors			;
	class AgcmUIManager2*		m_pAgcmUIManager2		;
	class AgcmUILogin*			m_pAgcmUILogin			;
	class AgcmUICharacter*		m_pAgcmUICharacter		;
	class AgcmOcTree*			m_pAgcmOcTree			;
	class ApmOcTree*			m_pApmOcTree			;
	class AgcmTextBoardMng*		m_pAgcmTextBoardMng		;
	class ApmMap*				m_pApmMap				;

	class AgcmShadow*			m_pAgcmShadow			;
	class AgcmReturnToLogin*	m_pAgcmReturnToLogin	;

	class AgpmSiegeWar*			m_pAgpmSiegeWar			;
	class AgcmSiegeWar*			m_pAgcmSiegeWar			;
...
//	// 이게 꼭 필요하다... 다른데서도 커서 등록할 수 있기 때문에.. (parn)
	...
	void EnableRMBDblClickMovement(BOOL bValue);
};
