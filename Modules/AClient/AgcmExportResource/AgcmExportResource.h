...
#include <AcuFileAdmin/AcuFileAdmin.h>
#include <AcuTexture/AcuTexture.h>
#include <AcuObject/AcuObject.h>
#include <AcuExportDFF/AcuExportDFF.h>
#include <AgcmEff2/AgcuEffIniMng.h>
...
struct AgcdObjectTemplateGroupList;
...
namespace TSO	// Template Separated Operating
{
	...
	BOOL	LoaderBase< T >::SaveTemplateOne( const char * pPath , T * ppdTemplate )
	{
		...
		{
			char strDebug[ 256 ] = { 0, };
			sprintf_s( strDebug, sizeof( char ) * 256, "템플릿( TID = %d )을 저장할 수 없습니다. 데이터를 확인해주세요.\n", ppdTemplate->m_lID );
		...
		else
		{
			...
			{
				char strDebug[ 256 ] = { 0, };
				sprintf_s( strDebug, sizeof( char ) * 256, "템플릿( TID = %d )을 저장하던 중 오류가 발생했습니다. ( 파일명 = %s )\n", ppdTemplate->m_lID, strWriteFileName );
				::MessageBox( NULL, strDebug, "Error!", MB_OK );
			}
		}
