...
//@} Jaewon
{
	...
	//@{ 20050513 DDonSS : Threadsafe 처리로 인해 인자 추가됨( 디버깅용.. 두번째 인자에 Lock 함수 지정할수 있도록 )
	// 여차 문제 있다 싶으면... rwENGINEINITNOFREELISTS 옵션 넣자.
	...
	//@{ 20050513 DDonSS : Threadsafe Test용.. 평상시에는 처리되지 않음..
...
/*
...

		{
			...
			ASSERT( dwFindIndex != -1 && "비디오모드 찾기 실패" );
			if( dwFindIndex == -1 )
			{
				TRACE( "비디오모드를 찾지 못해서 강제 종료함.\n" );
				return FALSE;
			}
		}
