...
#define AWS_FRAMED (WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX)
#define AWS_FRAMELESS WS_POPUP
...
static rwD3D9DeviceReleaseCallBack g_RwDeviceReleaseCB = NULL;
...
static AgcmImGui *	g_pcsAgcmImGui = NULL;
...
psCameraShowRaster(RwCamera *camera)
{
	...
	// full screen mode.
	//RwCameraShowRaster(camera, PSGLOBAL(window), PSGLOBAL(enableVSync) ? rwRASTERFLIPWAITVSYNC : 0);
...
InitInstance(HANDLE instance)
{
    ...
    RECT rect;
	//DWORD style = PsGlobal.framed ? AWS_FRAMED : AWS_FRAMELESS;
	DWORD style = AWS_FRAMED;
	HWND hwnd;
	...
	if (PsGlobal.framed) {
		...
		if (!SystemParametersInfoA(SPI_GETWORKAREA, 0, &wr, 0))
			wr = rect;
		if (wr.right - wr.left >= rect.right) {
			LONG pad = ((wr.right - wr.left) - (rect.right - rect.left)) / 2;
			LONG move = (wr.left + pad) - rect.left;

			rect.left += move;
			rect.right += move;
		}
		if (wr.bottom - wr.top >= rect.bottom) {
			LONG pad = ((wr.bottom - wr.top) - (rect.bottom - rect.top)) / 2;
			LONG move = (wr.top + pad) - rect.top;

			rect.top += move;
			rect.bottom += move;
		}
	}
    hwnd = CreateWindow(AppClassName, RsGlobal.appName,
		style,
		rect.left, 
		rect.top,
...
MainWndProc(HWND window, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	if (AgcmImGui::WndProc(window, message, wParam, lParam))
		return 0L;
    switch( message )
    {
#if !defined(UNDER_CE)
...
//				ms.control	= ( wParam & MK_CONTROL	) ? TRUE : FALSE;
		...
		case WM_IME_STARTCOMPOSITION:
			...
			AuInputComposer::GetInstance().AdjustCompositionWindowPos(window);
			return 0L;
...
adjust_resolution()
{
	...
	if (!AdjustWindowRect(&r, st, FALSE))
		return FALSE;
	w = r.right - r.left;
	h = r.bottom - r.top;
	if (RsGlobal.maximumWidth + w > (wr.right - wr.left))
		RsGlobal.maximumWidth = (wr.right - wr.left) - w;
	if (RsGlobal.maximumHeight + h > (wr.bottom - wr.top))
		RsGlobal.maximumHeight = (wr.bottom - wr.top) - h;
	return TRUE;
}
        ...
        int cmdShow)
{
	...
	*/
	...
	if (!PSGLOBAL(framed)) {
		...
		::SetWindowLongA(PsGlobal.window, GWL_STYLE, AWS_FRAMELESS);
		::SetWindowPos(PsGlobal.window, HWND_TOP, 
			rect.left, rect.top,
			rect.right - rect.left,
			rect.bottom - rect.top,
			SWP_FRAMECHANGED);
     ...
     */
    ShowWindow(PSGLOBAL(window), SW_SHOW);
    ...
    while (!RsGlobal.quit) {
		...
		if (PeekMessage(&g_msgLastReceived, NULL, 0U, 0U, PM_REMOVE)) {
