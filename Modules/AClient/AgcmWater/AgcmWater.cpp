...
BOOL	AgcmWater::CB_W2Render ( PVOID pData, PVOID pClass, PVOID pCustData )
{
	...
	{
		DrawWave*		nw_DrawWave = (DrawWave*)AcuFrameMemory::Alloc(sizeof(DrawWave));
...
/*		INT32	foffset = 0;
		...
		for(i=0;i<WaterNum;++i)		//	Water추가
		{
			...
			if (nw_Water->StatusID >= WMAP_STATUS_NUM) {
				delete nw_Water;
				continue;
			}
...
BOOL	AgcmWater::CB_HWRender ( PVOID pData, PVOID pClass, PVOID pCustData )
{
	...
	{
		DrawHWater*		nw_DH = (DrawHWater*)AcuFrameMemory::Alloc(sizeof(DrawHWater));
