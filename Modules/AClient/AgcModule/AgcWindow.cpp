...
/*
	...
	{
		//-----------------------------------------------------------------------
		// 현재 해상도에 따라 UI 위치를 조절합니다.
		//
		if( pWindow->pParent && !pWindow->pParent->pParent ) // 배경 ui의 자식이면
		{
			// 1024 768 기준으로 저장된 ui 좌표를 현재 해상도 기준의 좌표로 조정합니다.

			// ui의 중점을 기준으로 계산
			float centerX = (pWindow->x + pWindow->w)*0.5f;
			float centerY = (pWindow->y + pWindow->h)*0.5f;

			// x는 1024의 센터 512를 기준으로 조정합니다.
			float newCenterX = (centerX * (RsGlobal.maximumWidth*0.5f) / 512);
			pWindow->x += (INT16)(newCenterX - centerX);

			// y는 센터에서 가까운 가장자리를 기준으로 조정합니다.
