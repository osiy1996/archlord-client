...
#include <AgcmUIControl/AcUITree.h>
#include <AgcmItem/AgcmItem.h>

#define	BUTTON_COOLTIME												500
#define PROGRESS_COOLTIME											500
...
BOOL AgcmUIAuction::CBDisplayAuctionTree( PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue, AgcdUIControl *pcsSourceControl )
{
	if( pClass )
	{
		...
		else if( lDepth == AGCM_UI_AUCTION_DEPTH_2 )
		{
			...
			AgpdItemTemplate * pcsItemTemplate = 
				pThis->m_pcsAgpmItem->GetItemTemplate(lID);
            ...
            if (pcsItemTemplate) {
				if (pcsItemTemplate->m_nType == AGPMITEM_TYPE_EQUIP) {
					...
					pThis->m_pcsAgpmFactors->GetValue(
						&pcsItemTemplate->m_csRestrictFactor, 
						&nLevel, 
						AGPD_FACTORS_TYPE_CHAR_STATUS, 
						AGPD_FACTORS_CHARSTATUS_TYPE_LEVEL);
					sprintf_s(strDisplayData, sizeof(strDisplayData), 
						"%s%d %s", 
