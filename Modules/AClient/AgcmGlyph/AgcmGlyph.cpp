...
RpAtomic* AgcmGlyph::AtomicsRenderProcess(RpAtomic* pAtomic, void* pData)
{
	...
	if( pGeometry )
	{
		RwTexture**	TempTextures = (RwTexture**)AcuFrameMemory::Alloc(RpGeometryGetNumMaterials(pGeometry) * sizeof(RwTexture*));
		...
		AcuFrameMemory::Dealloc(TempTextures);
	}
...
static void *
reallocBlocking(void * p, size_t n)
{
	void * pMem = NULL;
...
void	AgcmGlyph::UpdateDestEnemyTarget(INT32	tickdiff)
{
		...
		!IsBadReadPtr( m_stDestEnemyTarget.movepos , sizeof RwFrame ) )
	{
		...
		if(res != rwSPHEREOUTSIDE)
		{
			...
			{
				m_stDestEnemyTarget.vertex[i] = (RwIm3DVertex*)reallocBlocking(
					m_stDestEnemyTarget.vertex[i], 
					m_stDestEnemyTarget.mem_alloc_size);
			...
			if(m_stDestEnemyTarget.eSkillType != DEST_TARGET_NONE)
			{
				...
				{
					m_stDestEnemyTarget.vertex_skill_deco = (RwIm3DVertex*)reallocBlocking(
						m_stDestEnemyTarget.vertex_skill_deco,
						m_stDestEnemyTarget.mem_alloc_size);
