...
/*
 * Export .ECL file in a custom format.
 * text: [FILENAME]
*/
static void
exportClump(const std::string & text, AgcdCharacterTemplate * character_template = NULL)
{
	AgcConsole * console = &((AgcmUIConsole *)g_pEngine->GetModule("AgcmUIConsole"))->getConsole();
	AgcmCharacter * cm = (AgcmCharacter *)g_pEngine->GetModule("AgcmCharacter");
	RpClump * clump;
	CHAR tmp[256];
	RwUInt32 index = 0;
	RpAtomic * atomic;
	FILE * file;
	RwUInt32 geometry_count = 0;
	RpHAnimHierarchy * hierarchy = NULL;
	int has_hierarchy = 0;

	console->print("Export_file = " + text);
	strcpy(tmp, text.c_str());
	strupr(tmp);
	clump = cm->LoadClump(tmp);
	if (clump == NULL) {
		console->print("AgcmCharacter::LoadClump() failed");
		return;
	}
	atomic = clump->atomicList;
	if (atomic == NULL) {
		console->print("Empty atomic list");
		RpClumpDestroy(clump);
		return;
	}
	do {
		if (atomic->geometry->mesh && atomic->geometry->mesh->numMeshes)
			geometry_count++;
		atomic = atomic->next;
	} while (atomic != clump->atomicList);
	sprintf(tmp, "D:\\ArchLord\\content\\Exports\\models\\%s.almodel", text.c_str());
	file = fopen(tmp, "wb");
	if (file == NULL) {
		console->print("Failed to open export file");
		RpClumpDestroy(clump);
		return;
	}
	RwFrameForAllChildren(RpClumpGetFrame(clump), findFrame, (void *)&hierarchy);
	if (hierarchy) {
		/*sprintf(tmp, "HAnim_NumNodes = %d", h->numNodes);
		console->print(tmp);
		for (RwInt32 i = 0; i < h->numNodes; i++) {
			RpHAnimNodeInfo * node = &h->pNodeInfo[i];

			sprintf(tmp, "HAnimNode_Index= %d\nHAnimNode_ID = %d\nHAnimNode_Flags = %08X", node->nodeIndex, node->nodeID, node->flags);
			console->print(tmp);
		}*/
		has_hierarchy = 1;
		fwrite(&has_hierarchy, 1, 1, file);
		fwrite(&hierarchy->numNodes, 4, 1, file);
		for (RwInt32 i = 0; i < hierarchy->numNodes; i++) {
			RpHAnimNodeInfo * node = &hierarchy->pNodeInfo[i];

			fwrite(&node->nodeIndex, 4, 1, file);
			fwrite(&node->nodeID, 4, 1, file);
		}
	}
	else {
		fwrite(&has_hierarchy, 1, 1, file);
	}
	fwrite(&geometry_count, 4, 1, file);
	atomic = clump->atomicList;
	do {
		RpGeometry * geo = atomic->geometry;
		
		if (geo->mesh == NULL || geo->mesh->numMeshes == 0) {
			continue;
		}
		/*sprintf(tmp, "GeometryInfo[%u]:", index);
		console->print(tmp);
		sprintf(tmp, "numMorphTargets = %d\nnumVertices = %d\nnumTriangles = %d\nnumTexCoordSets = %d", geo->numMorphTargets, geo->numVertices, geo->numTriangles, geo->numTexCoordSets);
		console->print(tmp);*/
		assert(geo->numVertices == 0 || geo->numMorphTargets != 0);
		fwrite(&geo->numVertices, 4, 1, file);
		fwrite(geo->morphTarget[0].verts, geo->numVertices * sizeof(RwV3d), 1, file);
		fwrite(geo->morphTarget[0].normals, geo->numVertices * sizeof(RwV3d), 1, file);
		fwrite(&geo->mesh->numMeshes, 2, 1, file);
		for (RwUInt16 i = 0; i < geo->mesh->numMeshes; i++) {
			RpMesh * mesh = (RpMesh *)((uintptr_t)geo->mesh + sizeof(RpMeshHeader) + geo->mesh->firstMeshOffset + i * sizeof(RpMesh));

			fwrite(&mesh->numIndices, 4, 1, file);
			fwrite(mesh->indices, mesh->numIndices * sizeof(RxVertexIndex), 1, file);
		}
		atomic = atomic->next;
		index++;
	} while (atomic != clump->atomicList);
	...
	/*if (character_template) {
		INT32				lNumAnimType2 = cm->GetAnimType2Num(character_template);
		...
		for (INT32 lAnimType = lStartAnimType; lAnimType < lEndAnimType; ++lAnimType) {
			for (INT32 lAnimType2 = lStartAnimType2; lAnimType2 < lEndAnimType2; ++lAnimType2) {
				AgcdCharacterAnimation * anim = character_template->m_pacsAnimationData[lAnimType][lAnimType2];

				if (anim == NULL || anim->m_pcsAnimation == NULL)
					continue;
			}
		}
	}*/
...
/*
 * Export .ECL file in a custom format.
 * n: Clump file id. Ex: exportClump(1) -> exportClump("B0000001.ECL")
*/
static void
exportClumpUInt(const RwUInt32 n) {
	char tmp[128];
	AgcmCharacter * cm = (AgcmCharacter *)g_pEngine->GetModule("AgcmCharacter");
	AgpdCharacterTemplate * t = ((AgpmCharacter *)g_pEngine->GetModule("AgpmCharacter"))->GetCharacterTemplate(n);
	AgcdCharacterTemplate * ct = NULL;

	if (t)
		ct = cm->GetTemplateData(t);
	sprintf(tmp, "B%07u.ECL", n);
	exportClump(tmp, ct);
}
...
{
	AgcmCharacter * cm = (AgcmCharacter *)g_pEngine->GetModule("AgcmCharacter");
	AgcdCharacter * self = cm->GetCharacterData(cm->GetSelfCharacter());
	RwV3d scale = { 10.f, 10.f, 10.f };

	self->m_pstAgcdCharacterTemplate->m_fScale = 10.f;
	self->m_pstAgcdCharacterTemplate->m_stScale = scale;
...
BOOL AgcmUIConsole::OnInit()
{
	...
	result = getScriptEngine().registerGlobalFunctionCdecl("void exportClump(string &in)", exportClump);
	assert(result);
	result = getScriptEngine().registerGlobalFunctionCdecl("void exportClump(uint)", exportClumpUInt);
	assert(result);
	getConsole().registerKeyword("exportClump( )");

	result = getScriptEngine().registerGlobalFunctionCdecl("void setSelfScale()", setSelfScale);
	assert(result);
	getConsole().registerKeyword("setSelfScale()");
