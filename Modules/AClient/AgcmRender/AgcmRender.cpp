...
#include <AgcmSkill/AgcmSkill.h>	//for debugging skill
#include <AgcmObject/AgcmObject.h>	//for show object collision data
...
{
	Shadow2TerrainList*		nw_node = (Shadow2TerrainList*)AcuFrameMemory::Alloc(sizeof(Shadow2TerrainList));
...
RpAtomic*	AgcmRender::RenderCallbackForAmbientAtomic	( RpAtomic *atomic )
{
	...
	//AmbientList에 추가
	TextureSortAtomic*	new_node = (TextureSortAtomic*)AcuFrameMemory::Alloc(sizeof(TextureSortAtomic));
...
//#endif
	...
	//Skin List에 추가
	new_node = (SkinningTSortAtomic*)AcuFrameMemory::Alloc(sizeof(SkinningTSortAtomic));
...
RpAtomic*	AgcmRender::RenderCallbackForSkinPipeAlpha( RpAtomic* atomic )
{
	...
	SkinningAtomic*	new_node = (SkinningAtomic*)AcuFrameMemory::Alloc(sizeof(SkinningAtomic));
...
RpAtomic*	AgcmRender::RenderCallbackForSkinPipeAlphaFunc(RpAtomic*	atomic)
{
	...
	//Skin AlphaFunc에 추가
	SkinningTSortAtomic* new_node = (SkinningTSortAtomic*)AcuFrameMemory::Alloc(sizeof(SkinningTSortAtomic));
...
RpAtomic*	AgcmRender::RenderCallbackForUVAnim(RpAtomic*	atomic)
{
	...
	//uvanim list에 추가
	UVAnimAtomic*	new_node = (UVAnimAtomic*)AcuFrameMemory::Alloc(sizeof(UVAnimAtomic));
...
{
	//m_pThisAgcmRender->AtomicPreRender(atomic);
	SimpleAtomic*	new_node = (SimpleAtomic*)AcuFrameMemory::Alloc(sizeof(SimpleAtomic));
...
SectorData * AgcmRender::GetClientSectorData(ApWorldSector * pSector)
{
	return (SectorData*)m_pcApmMap->GetAttachedModuleData(m_iSectorDataIndex, (PVOID)pSector);
}

SectorRenderList * AgcmRender::GetSectorData(ApWorldSector * pSector)
{
	return (SectorRenderList*)m_pcApmMap->GetAttachedModuleData(m_iSectorDataIndex, (PVOID)pSector);
}
...
void	AgcmRender::CullModeWorldRenderGeometryOnly( int type )		// 0 - 지형 1- 그외(지형포함)
{
	...
	for(int i=sx;i<ex;++i)
	{
		...
		for(int j=sz;j<ez;++j)
		{
			...
			// 지형 그리기
			if (type <= 1) {
				nw_node = (Shadow2TerrainList*)AcuFrameMemory::Alloc( SHADOW2_TERRAIN_LIST_SIZE );
...
void	AgcmRender::InsertPreListOcTree(RpAtomic* atomiclist,RpClump*	pClumpArg)
{
	...
	PreRender*	add_node = (PreRender*) AcuFrameMemory::Alloc(sizeof(PreRender));
	...
	if(pData->bMove)
	{
		...
		if(pData->isOccluder && m_fPanAngle < 26.0f && m_fPanAngle > -20.0f) // 카메라가 높은 곳에서 아래를 쳐다볼경우 성벽 같은데서 지평선 구성시 오류가 생길수 있다
		{
			...
			add_node->arrayOccluder = (FLOAT*) AcuFrameMemory::Alloc(12*pData->nOccluderBox); 
...
void	AgcmRender::InsertPreListCustom( OcCustomDataList*	customdata )
{
	...
	PreRender*	add_pr = (PreRender*) AcuFrameMemory::Alloc(sizeof(PreRender));
...
void	AgcmRender::UpdateOBufferAndDrawTest(AgcmRenderTestRenderAtomicList		pRenderFunc,BOOL bCustProcess)
{
	...
	OccluderLineList*	add_Onode = (OccluderLineList*) AcuFrameMemory::Alloc(sizeof(OccluderLineList));
...
void	AgcmRender::OcTreeModeRender(RwCamera*	pCheckCamera,AgcmRenderTestRenderTerrain	pTerrainFunc,BOOL bWorldAdded)
{
	...
	for(i=sx;i<ex;++i)
	{
		...
		for(j=sz;j<=ez;++j)
		{
			...
			if(res != rwSPHEREOUTSIDE )
			{
				...
				//@{ Jaewon 20050714
				// PreRenderList -> std::priority_queue<PreRender>
				add_terrain = (PreRender*) AcuFrameMemory::Alloc(sizeof(PreRender));
				...
				if(m_fPanAngle < 70.0f && m_fPanAngle > -25.0f && m_bOcLineCheck && nDistSector < 4)
				{
					...
					add_terrain->arrayOccluder = (FLOAT*) AcuFrameMemory::Alloc(768); // 12 * quad tree node수(64) = 1024
					terrain_OBoxindex = 0;
					// Occluder data set
					for(k=0;k<4;++k)
					{
	...
	while(cur_cr)
	{
		...
		if(res != rwSPHEREOUTSIDE)
		{
			//@{ Jaewon 20050714
			// PreRenderList -> std::priority_queue<PreRender>
			add_pr = (PreRender*) AcuFrameMemory::Alloc(sizeof(PreRender));
			ZeroMemory( add_pr , sizeof(PreRender) );
			//@} Jaewon
	...
	while(sx<ex && cur_node)
	{
		...
		else if(cur_node->startx == sx)
		{
			...
			else if(cur_node->endx > ex)				// 현재 노드와 시작 지점이 같고 넓이가 더 짧은경우
			{
				if(cur_node->y > y)
				{
					add_Onode = (OccluderLineList*) AcuFrameMemory::Alloc(sizeof(OccluderLineList));
					add_Onode->startx = ex;
		...
		else if(cur_node->startx < sx)
		{
			...
			else if(cur_node->endx > ex)
			{
				if(cur_node->y > y)
				{
					...
					add_Onode2 = (OccluderLineList*) AcuFrameMemory::Alloc(sizeof(OccluderLineList));
...
RpAtomic*	AgcmRender::RenderCallbackForVSFadeAtomic( RpAtomic* pAtomic )
{
	...
	// 매프레임마다 동적추가
	VShaderFadeAtomic*	new_fade = (VShaderFadeAtomic*)AcuFrameMemory::Alloc(sizeof(VShaderFadeAtomic));
	stBlendAtomic NewBlendAtomic;
	...
	if( shader_type == VS_FADEINOUT_A )
	{
		if(per<0.5f)
		{
			...
			new_fade = (VShaderFadeAtomic*)AcuFrameMemory::Alloc(sizeof(VShaderFadeAtomic));
...
void	AgcmRender::ProcessFadeAtomic( RpAtomic* atomic, VShaderFadeAtomic*	pData )
{
	...
	RpMaterial*		pMat;
	int	i;
	void *		pAllocHead = AcuFrameMemory::Alloc(sizeof(RwRGBA) * mat_num);
	RwRGBA*		matColorSrcArray = (RwRGBA*)pAllocHead;
	...
	if( ACUOBJECT_TYPE_CHARACTER == iType || ACUOBJECT_TYPE_ITEM == iType )
	{
		...
		{
			orgColor = (D3DXVECTOR4*)AcuFrameMemory::AllocAppend(sizeof(D3DXVECTOR4) * mat_num);
			for(i =0;i<mat_num;++i)
			{
				pMat = RpGeometryGetMaterial (pGeom,i);
	...
	AcuFrameMemory::Dealloc(pAllocHead);
}
