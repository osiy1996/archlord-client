...
#define AGPMITEM_LOTTERY_BOX_CONVERSION_DURATION_MSEC	1000
...
AgcmItem::AgcmItem() 
{
	...
	memset(m_ulItemDropColor, 0, sizeof(m_ulItemDropColor));
}
...
BOOL AgcmItem::OnInit()
{
	...
	if (!ReadItemDropSettings())
		return FALSE;
	if (!m_ItemEffectFinder.ReadFile())
		return FALSE;
	return TRUE;
}
...
BOOL AgcmItem::LoadTemplateTexture(AgpdItemTemplate *pcsAgpdItemTemplate, AgcdItemTemplate *pstAgcdItemTemplate)
{
	...
	if (pszSmallTexture)
	{
		...
		if (m_pcsAgcmUIControl)
		{
			...
			if( ppRwTexture && (*ppRwTexture) && pstAgcdItemTemplate->m_pSmallTexture )
...
BOOL
AgcmItem::ReadItemDropSettings()
{
	AuExcelLib * pExcel = AuExcel::LoadExcelFile(
		"INI\\itemdropinfo.txt", FALSE);
	int lRowCount;

	if (!pExcel) {
		OutputDebugStringA(
			"AgcmItem::LoadItemDrop(): failed to open file.\n");
		return FALSE;
	}
	lRowCount = pExcel->GetRow();
	for (int i = 0; i < lRowCount; i++) {
		const char * pszData = pExcel->GetData(0, i);
		AgcdItemDropCategoryId eId = AGCD_ITEM_DROP_CATEGORYID_COUNT;
		UINT32 ulItemTemplateID = 0;
		
		sscanf(pszData, "%u=%u:", &eId, &ulItemTemplateID);
		if (m_hmapItemDropCategory.find(ulItemTemplateID) ==
			m_hmapItemDropCategory.end())
			m_hmapItemDropCategory.insert(
				std::make_pair(ulItemTemplateID, eId));
	}
	delete pExcel;
	pExcel = AuExcel::LoadExcelFile(
		"INI\\itemdropconfig.ini", FALSE);
	if (!pExcel) {
		OutputDebugStringA(
			"AgcmItem::LoadItemDrop(): failed to open file.\n");
		return FALSE;
	...
	for (int i = 0; i < lRowCount; i++) {
		...
		if (strncmp(pszData, "Enable=", 7) == 0) {
			m_bEnableItemDropSettings =
				(strtol(strchr(pszData, '=') + 1, NULL, 10) != 0);
		}
		else if (strncmp(pszData, "Color=", 6) == 0) {
			AgcdItemDropCategoryId eId = AGCD_ITEM_DROP_CATEGORYID_COUNT;
			UINT32 ulColor;
			
			sscanf(strchr(pszData, '=') + 1, "%u:%u", &eId, &ulColor);
			if (eId < AGCD_ITEM_DROP_CATEGORYID_COUNT)
				m_ulItemDropColor[eId] = ulColor;
		}
	}
	delete pExcel;
	return TRUE;
...
BOOL
AgcmItem::WriteItemDropSettings()
{
	FILE * file = fopen("INI\\itemdropconfig.ini", "w");

	if (!file)
		return FALSE;
	fprintf(file, "Enable=%d\n", m_bEnableItemDropSettings);
	for (UINT32 i = 0; i < AGCD_ITEM_DROP_CATEGORYID_COUNT; i++)
		fprintf(file, "Color=%u:%u\n", i, m_ulItemDropColor[i]);
	fclose(file);
	return TRUE;
}

void
AgcmItem::EnableItemDropSettings(BOOL bEnabled)
{
	m_bEnableItemDropSettings = bEnabled;
}

BOOL
AgcmItem::IsItemDropSettingsEnabled()
{
	return m_bEnableItemDropSettings;
}

void
AgcmItem::SetItemDropColor(AgcdItemDropCategoryId eCategory, UINT32 ulColor)
{
	if (eCategory < AGCD_ITEM_DROP_CATEGORYID_COUNT)
		m_ulItemDropColor[eCategory] = ulColor;
}

UINT32
AgcmItem::GetItemDropColor(AgcdItemDropCategoryId eCategory)
{
	if (eCategory < AGCD_ITEM_DROP_CATEGORYID_COUNT)
		return m_ulItemDropColor[eCategory];
	else
		return 0;
}

UINT32
AgcmItem::GetItemDropColor(UINT32 ulItemTemplateID)
{
	ItemDropCategoryMap::const_iterator it = 
		m_hmapItemDropCategory.find(ulItemTemplateID);

	if (it != m_hmapItemDropCategory.end() && 
		it->second < AGCD_ITEM_DROP_CATEGORYID_COUNT)
		return m_ulItemDropColor[it->second];
	return 0;
}
