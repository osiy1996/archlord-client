...
BOOL AgcmCharacter::StreamReadPolyMorphTable( CHAR *szFile, CHAR *pszErrorMessage, BOOL bDecryption )
{
	...
	for(INT16 nRow = 1; nRow < csExcelTxtLib.GetRow(); ++nRow)
	{
		...
		stCharacterPolyMorphEntry NewEntry;

		for( int nCol = 1 ; nCol < csExcelTxtLib.GetColumn() ; nCol++ )
		{
			pszData = csExcelTxtLib.GetData( nCol, nKeyRow );
			if( pszData )
			{
				if( strcmp( pszData, "Condition" ) == 0 )
				{
					pszData = csExcelTxtLib.GetData( nCol, nRow );
					if( pszData )
					{
						NewEntry.m_eConditionType = ( AgcmCharacterPolymorphCondition )atoi( pszData );
					}
				}
				else if( strcmp( pszData, "FirstTID" ) == 0 || strcmp( pszData, "SecondTID" ) == 0 || strcmp( pszData, "ThirdTID" ) == 0 )
				{
					pszData = csExcelTxtLib.GetData( nCol, nRow );
					if( pszData )
					{
						int nTID = atoi( pszData );
						NewEntry.m_vecTID.Add( nTID );
					}
				}
			}
		}
		...
		m_PolyMorphTableMap.Add( iBaseTID, NewEntry );
	}
