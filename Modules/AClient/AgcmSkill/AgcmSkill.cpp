...
#include <AgpmProduct/AgpdProduct.h>
...
BOOL AgcmSkill::ProcessReceiveCastSkill(pstAgpmSkillActionData pstActionData)
{
	...
	AgpdSkillTemplate*		pTemplate		=	m_pcsAgpmSkill->GetSkillTemplate( pstActionData->lSkillTID );
...
//			break;
	...
	*/
	...
	if (!SendPacket(pvPacket, nPacketLength)) {
		...
		AgpdCharacter * pcsChar = m_pcsAgcmCharacter->GetSelfCharacter();

		//if (bInRange) {
		//	m_pcsAgpmCharacter->StopCharacter(pcsChar, NULL);
		//	m_pcsAgcmCharacter->StopSelfCharacter();
		//}
		m_pcsAgpmCharacter->SetMoveLockFrameTick(g_pEngine->GetFrameTick());
	}
	//else {
	//	m_pcsAgpmCharacter->SetActionBlockTime(
	//		m_pcsAgcmCharacter->m_pcsSelfCharacter, 
	//		800, 
