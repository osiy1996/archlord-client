...
#include <AgcmUISkill2/AgcmUISkillDragonScionDef.h>
...
: m_pItemTooltip(NULL)
{
	...
	m_privateTradeSelfCCUD = NULL;
	m_privateTradePeerCCUD = NULL;
	...
	m_privateTradeSelfCC = 0;
	m_privateTradePeerCC = 0;
...
BOOL AgcmUIItem::OnAddModule()
{
	...
	if (!m_pcsAgpmGrid->Init(&m_csItemLockGrid, 1, 1, 1))
		return FALSE;
	...
	if(!m_pcsAgcmPrivateTrade->SetCallbackUpdateTargetTradeMoney( CBPrivateTradeUpdateClientGhelld, this ) )
		return FALSE;
	if(!m_pcsAgcmPrivateTrade->SetCallbackUpdateTradeSelfCC(
		CBPrivateTradeUpdateSelfCC, this ))
		return FALSE;
	if(!m_pcsAgcmPrivateTrade->SetCallbackUpdateTradePeerCC(
		CBPrivateTradeUpdatePeerCC, this ))
		return FALSE;
...
BOOL AgcmUIItem::AddFunction()
{
	...
	if (!m_pcsAgcmUIManager2->AddFunction(this, 
		"PT_UpdateCC", CBPrivateTradeUpdateCC, 1 ))
		return FALSE;
	...
	if (!m_pcsAgcmUIManager2->AddFunction( this, "Item_GetBtnAutoPickUp", CBGetBtnAutoPickUp, 0 ) )
		return FALSE;
	if (!m_pcsAgcmUIManager2->AddFunction(this, 
		"ItemLock_MoveItem", CBItemLockMoveItem, 1))
		return FALSE;
	if (!m_pcsAgcmUIManager2->AddFunction(this, 
		"ItemLock_Confirm", CBItemLockConfirm, 1))
		return FALSE;
	return TRUE;
}
...
BOOL AgcmUIItem::AddDisplay()
{
	...
	if (!m_pcsAgcmUIManager2->AddDisplay(this, 
		"PrivateTradeSelfCC", AGCMUI_ITEM_DISPLAY_ID_PT_SELF_CC, 
		CBDisplayPrivateTradeSelfCC, AGCDUI_USERDATA_TYPE_INT32))
		return FALSE;
	if (!m_pcsAgcmUIManager2->AddDisplay(this, 
		"PrivateTradePeerCC", AGCMUI_ITEM_DISPLAY_ID_PT_PEER_CC, 
		CBDisplayPrivateTradePeerCC, AGCDUI_USERDATA_TYPE_INT32))
		return FALSE;
...
BOOL AgcmUIItem::AddUserData()
{
	...
	*/
	...
	if ( !(m_pcsPrivateTradeClientGhelld = m_pcsAgcmUIManager2->AddUserData( "PrivateTradeClientGhelld", &m_lPrivateTradeClientGhelld, sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	if ( !(m_privateTradeSelfCCUD = m_pcsAgcmUIManager2->AddUserData(
		"PrivateTradeSelfCC", &m_privateTradeSelfCC, 
		sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	if ( !(m_privateTradePeerCCUD = m_pcsAgcmUIManager2->AddUserData(
		"PrivateTradePeerCC", &m_privateTradePeerCC, 
		sizeof(INT32), 1, AGCDUI_USERDATA_TYPE_INT32)))
		return FALSE;
	...
	*/
	...
	if (!(m_pcsSubInventory = m_pcsAgcmUIManager2->AddUserData("SubInventoryGrid", m_pcsSubInventoryGrid, sizeof(AgpdGrid), 1, AGCDUI_USERDATA_TYPE_GRID)))
		return FALSE;
	if (!(m_pcsItemLockGridUD = m_pcsAgcmUIManager2->AddUserData(
		"ItemLock_Grid", 
		&m_csItemLockGrid, sizeof(AgpdGrid), 
		1, AGCDUI_USERDATA_TYPE_GRID)))
		return FALSE;
...
BOOL AgcmUIItem::CBUseItemResult(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	case AGPMITEM_USE_RESULT_CANNOT_USE_TRANSFORM_POTION:
		{
			pThis->m_pcsAgcmChatting2->AddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_USE_ITEM_RESULT_CANNOT_USE_TRANSFORM_POTION));
		}
		break;
	}
			...
			)
		{
			...
			if (pcsGridItemLotteryKey == NULL)
			{
				...
				if (pcsItem->m_pcsItemTemplate->m_nSubType == AGPMITEM_USABLE_LOTTERY_TYPE_GOLD)
					...
					pThis->m_pcsAgcmChatting2->AddSystemMessage(pThis->m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_LOTTERYBOX_NEEDS_100BAG_KEY));
				#endif
...
BOOL AgcmUIItem::CBDisplayMessageData(PVOID pClass, PVOID pData, AgcdUIDataType eType, INT32 lID, CHAR *szDisplay, INT32 *plValue)
{
	AgcmUIItem *			pThis = (AgcmUIItem *)pClass;
	AgpdItem *				pcsItem;
	AgcmUIItemMessageData *	pstMsg;

	if (!pData ||
		eType != AGCDUI_USERDATA_TYPE_STRING ||
		!szDisplay)
		return FALSE;
	pstMsg = (AgcmUIItemMessageData *)pData;
	pcsItem = pThis->m_pcsAgpmItem->GetItem(pstMsg->lPickupItemID);
	switch (lID) {
	case AGCMUI_ITEM_DISPLAY_ID_MESSAGE_PICKUP_ITEM_NAME: {
			AgpdItemTemplate * pcsItemTemplate = 
				pThis->m_pcsAgpmItem->GetItemTemplate(pstMsg->lPickupItemTID);

			if (pcsItemTemplate) {
				if (pcsItemTemplate->m_lID == pThis->m_pcsAgpmItem->GetMoneyTID()) {
					sprintf(szDisplay, "%d %s", 
						pstMsg->lPickupItemCount, 
						ClientStr().GetStr(STI_GELD_POST));
				}
				else if (pcsItemTemplate->m_bStackable) {
					sprintf(szDisplay, "%s %d %s", 
						pcsItemTemplate->m_szName, 
						pstMsg->lPickupItemCount, 
						ClientStr().GetStr(STI_UNIT_POST));
				}
				else {
					if (pcsItem) {
						UINT32 ulColor = 
							pThis->GetTooltipColor(
								AGCMUIITEM_TOOLTIP_COLOR_NORMAL);
						UINT32 ulDropColor = 
							pThis->m_pcsAgcmItem->GetItemDropColor(
								pstMsg->lPickupItemTID);
						INT32 lItemRank	= 0;

						pThis->m_pcsAgpmFactors->GetValue(
							&pcsItem->m_pcsItemTemplate->m_csFactor, 
							&lItemRank, 
							AGPD_FACTORS_TYPE_ITEM, 
							AGPD_FACTORS_ITEM_TYPE_RANK);
						if (ulDropColor)
							ulColor = ulDropColor;
						else if (pcsItemTemplate->m_ulTitleFontColor > 0)
							ulColor	= pcsItemTemplate->m_ulTitleFontColor;
						else if (pcsItemTemplate->m_nType == AGPMITEM_TYPE_EQUIP && lItemRank == 2)
							ulColor	= pThis->GetTooltipColor(AGCMUIITEM_TOOLTIP_COLOR_RARE);
						else if (pcsItemTemplate->m_nType == AGPMITEM_TYPE_EQUIP && lItemRank == 3)
							ulColor = pThis->GetTooltipColor(AGCMUIITEM_TOOLTIP_COLOR_UNIQUE);
						else if (pcsItem->m_apcsOptionTemplate[0])
							//ulColor = AGCMUIITEM_TOOLTIP_COLOR_OPTION;	// 옵션이 있는 노멀 아이템을 하얀색으로 바꿨다
							ulColor = pThis->GetTooltipColor(AGCMUIITEM_TOOLTIP_COLOR_NORMAL);
						else if (pThis->m_pcsAgpmItemConvert->IsEgoItem(pcsItem))
							ulColor	= pThis->GetTooltipColor(AGCMUIITEM_TOOLTIP_COLOR_EGO);
						else if (pThis->m_pcsAgpmItemConvert->GetNumConvertedSocket(pcsItem) > 0)
							ulColor	= pThis->GetTooltipColor(AGCMUIITEM_TOOLTIP_COLOR_SPIRIT);

						// Item Name
						sprintf(szDisplay, "<C%d>%s", ulColor, pcsItemTemplate->m_szName);
