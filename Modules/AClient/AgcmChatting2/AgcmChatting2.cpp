...
#include <AgcmCamera2/AgcuCamMode.h>
...
BOOL AgcmChatting2::OnInit(VOID)
{
	...
	g_aulDefaultChatColor[AGPDCHATTING_TYPE_RACE]			=	pLoader->GetColor( "RACE_TRUMPET" );
	g_aulDefaultChatColor[AGPDCHATTING_TYPE_SCREAM]			=	pLoader->GetColor( "WHOLE_WORLD" );
	return TRUE;
}
...
BOOL AgcmChatting2::AddChatMessage(AgcmChattingType eChatType, AgpdChatData * pstChatData, BOOL bIsSendWhisper, BOOL bTranslateMessage)
{
	...
	case AGCMCHATTING_TYPE_WHISPER:
		...
		if (bTranslateMessage)
			...
			CopyMemory(szBuffer + strlen(szBuffer), pstChatData->szMessage, sizeof(CHAR) * pstChatData->lMessageLength);
		break;
	case AGCMCHATTING_TYPE_AREA:
		szMessage = m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_CHAT_AREA_HEADER);
		if (szMessage) {
			sprintf(szBuffer, "%s ", szMessage);
			sprintf(szBuffer + strlen(szBuffer), "%s : ", pstChatData->szSenderName);
		}
		if (bTranslateMessage)
			...
			CopyMemory(szBuffer + strlen(szBuffer), pstChatData->szMessage, sizeof(CHAR) * pstChatData->lMessageLength);
		ulColor	= g_aulDefaultChatColor[AGPDCHATTING_TYPE_WORD_BALLOON];
		break;
...
BOOL AgcmChatting2::AddAreaChatMessage(
	CHAR *	szMessage, 
	...
	UINT32	ulMessageType)
{
	...
	if (!szMessage || !szMessage[0] || lMessageLength < 1 || 
		ulMessageType >= AGPDCHATTING_TYPE_MAX)
		return FALSE;

	ZeroMemory(szBuffer, sizeof(CHAR) * 1100);
	if (bIsAddHeader)
		sprintf(szBuffer, "%s ", m_pcsAgcmUIManager2->GetUIMessage(UI_MESSAGE_ID_CHAT_AREA_HEADER));
	...
	if (bIsAdminCharacter)
		...
		AddMessage(AGCMCHATTING_TYPE_AREA, szBuffer, g_aulDefaultChatColor[ulMessageType], FALSE, TRUE);
	EnumCallback(AGCMCHATTING2_CB_INPUT_MESSAGE, NULL, NULL);
	return TRUE;
}
...
BOOL AgcmChatting2::CBRecvWordBalloonMsg(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	pThis->AddChatMessage(AGCMCHATTING_TYPE_AREA, pstChatData, FALSE, TRUE);
	pThis->EnumCallback(AGCMCHATTING2_CB_INPUT_MESSAGE, pstChatData, NULL);
	return TRUE;
}
...
BOOL AgcmChatting2::CBRecvAreaChatMsg(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	INT8			cIsAdmin;
	CHAR *			szMessage;
	UINT16			unMessageLength;
	INT8			cIsAddHeader;
	UINT32			ulMessageType;

	if (!pThis || !ppvBuffer)
		return FALSE;
	cIsAdmin		= (INT8)	ppvBuffer[0];
	szMessage		= (CHAR *)	ppvBuffer[1];
	unMessageLength	= (UINT16)	ppvBuffer[2];
	cIsAddHeader	= (INT8)	ppvBuffer[3];
	ulMessageType	= (UINT32)	ppvBuffer[4];
	if (!szMessage ||
		!szMessage[0] ||
		unMessageLength < 1 ||
		unMessageLength >= 1024)
		return FALSE;
	return pThis->AddAreaChatMessage(szMessage, unMessageLength, 
		(BOOL) cIsAdmin, (BOOL) cIsAddHeader, ulMessageType);
}
...
void Display_System_Message(char * pStr, UINT32 ulColor)
{
	AgcmChatting2 * pcsAgcmChatting2 = ( AgcmChatting2 * ) g_pEngine->GetModule( "AgcmChatting2" );
	if( pcsAgcmChatting2  )
	{
