...
BOOL AgcmReturnToLogin::CBReceiveAuthKey(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if( pcmUIMain )
	{
		pcmUIMain->OnClearSystemGrid();
	}

	AgcmReturnToLogin	*pThis	= (AgcmReturnToLogin *)	pClass;
	INT32	lAuthKey	= *(INT32 *) pData;
	CHAR	*pszLoginServerAddr	= (CHAR *) pCustData;
	AgcEngine * pEngine = (AgcEngine *)pThis->GetModuleManager();

	pEngine->m_SocketManager.SetDestroyNormal();
	pEngine->m_SocketManager.Disconnect(
		pEngine->m_SocketManager.GetGameServerNID());
