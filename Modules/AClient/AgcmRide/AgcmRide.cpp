...
BOOL AgcmRide::CBRideDismountAck(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	// 2010.10.13 kdi 장착해제를 서버에 알립니다. //--------
	AgpdGridItem* pRideGridItem = m_pThis->m_pcsAgpmItem->GetEquipItem( pcsAgpdCharacter, AGPMITEM_PART_RIDE );
	if( pRideGridItem ) {
		AgpdItem* pcsAgpdItemRide = m_pThis->m_pcsAgpmItem->GetItem( pRideGridItem->m_lItemID );
		if( pcsAgpdItemRide && pThis->m_pcsAgcmItem ) {
			pThis->m_pcsAgcmItem->SendInventoryInfo( pRideGridItem->m_lItemID
				, pcsAgpdItemRide->m_ulCID, m_pThis->m_pcsAgcmCharacter->m_lSelfCID );
		}
	}
	//--------
