...
#define AGCMUICHARACTER_NUM_TARGET_BUFFED_GRID	7
...
BOOL AgcmUICharacter::OnInit()
{
	...
	m_pcsAgpmGrid->Init(&m_csTargetBuffGrid, 1, 10, AGCMUICHARACTER_NUM_TARGET_BUFFED_GRID);
...
BOOL AgcmUICharacter::CharacterUpdateFactorBonusExpCB(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if( !ppmFactor || !ppmBillInfo ) return FALSE;

	BOOL bIsPCPang = ppmBillInfo->IsPCBang( pcsSelfCharacter );

	float fBonusExp = 0;
	ppmFactor->GetValue( pcsFactor, &fBonusExp, AGPD_FACTORS_TYPE_RESULT, AGPD_FACTORS_TYPE_CHAR_POINT, AGPD_FACTORS_CHARPOINT_TYPE_BONUS_EXP );
	if( fBonusExp <= 0.0f ) return TRUE;

	// 팩터에서 값을 가져올때 11 이 10.9 와 같은 형태로 나올때가 있다.. 그래서 소수점 체크해서 반올림 해주자..
	int nBonusExp = fBonusExp - ( float )( ( int )fBonusExp ) >= 0.5f ? ( int )fBonusExp + 1 : ( int )fBonusExp;
	int nBonusExpPCBang = bIsPCPang ? nBonusExp / 11 : 0;

	// Get Bonus Exp. Display message about get bonus exp.
	pThis->m_stMessageData.lBonusExp = nBonusExp;
	pThis->m_stMessageData.lBonusPCBangExp = nBonusExpPCBang;
...
BOOL AgcmUICharacter::CBUpdateActionStatus(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (pcsSelfCharacter == pcsCharacter)
	{
		if (pcsSelfCharacter->m_unActionStatus == AGPDCHAR_STATUS_DEAD)
		{
			...
			if (pcsCharacter->m_nBindingRegionIndex != 92)
				pThis->m_pcsAgcmUIManager2->ThrowEvent(
					pThis->m_lEventOpenResurrectionUI);
...
BOOL AgcmUICharacter::CBRefreshTargetName(PVOID pClass, PVOID pData1, PVOID pData2, PVOID pData3, PVOID pData4, PVOID pData5, ApBase *pcsTarget, AgcdUIControl *pcsSourceControl)
{
		...
		pThis->m_pcsApmMap->CheckRegionPerculiarity(  pThis->m_pcsTargetCharacter->m_nBindingRegionIndex , APMMAP_PECULIARITY_SHOWNAME ) == APMMAP_PECULIARITY_RETURN_DISABLE_USE )
	{
		...
		for (size_t i=0; i < strlen(pThis->m_pcsTargetCharacter->m_szID); ++i)
			szCharacterName[i] = '*';
	...
	else
	{
		...
		{
			if(pThis->m_pcsAgpmCharacter->IsMonster(pThis->m_pcsTargetCharacter) || g_eServiceArea == AP_SERVICE_AREA_GLOBAL)
				bDisplayLevel = TRUE;
		}
...
AgcdUICharacterADBase *	AgcmUICharacter::GetBaseData(ApBase *pcsBase, AuPOS *pstPos)
{
...
AgcdUICharacterADCharacterTemplate *	AgcmUICharacter::GetCharacterTemplateData(AgpdCharacterTemplate *pcsCharacterTemplate)
{
...
#else
	m_pcsTransformCancelGridItem->SetTooltip( ClientStr().GetStr( STI_ALARM_TRANSFORM ) );
#endif
...
#else
	m_pcsEvolutionCancelGridItem->SetTooltip( ClientStr().GetStr( STI_ALARM_TRANSFORM ) );
#endif
