...
BOOL AgcmUICharacterCreate::CB_OnCharacterCreateOK( void* pClass, void* pData1, void* pData2, void* pData3, void* pData4, void* pData5, ApBase* pTarget, AgcdUIControl* pControl )
{
	...
	AgpmCharacter * pmCharactor = (AgpmCharacter*)g_pEngine->GetModule( "AgpmCharacter" );

	if( pmCharactor && !pmCharactor->CheckCharName( strName, FALSE ) )
	{
		pcmUIManager->ThrowEvent( pcmUILogin->m_lEvent[ AGCMUILOGIN_EVENT_INVALID_CHAR_NAME_INPUT ] );
		return FALSE;
	}
	...
	pcmLogin->SetMakingCharacterInfo(ppdCharacter->m_lTID1, pThis->GetSelectFace(), pThis->GetSelectHair(), strName);
...
#ifndef _AREA_GLOBAL_ // 글로벌의 경우 결과 패킷을 한번 더 받고 처리함
