...
RwInt32 Eff2Ut_ForAllFile( const RwChar* path, const RwChar* exp, fptrProcess pProcess, void* lpOwerClass)				  
{
	...
	wchar_t const * folderInfo = GetZpackFolderInfo( L"effect.zp", path );

	if( folderInfo && wcslen( folderInfo ) )
	{
		std::wstringstream stream(folderInfo);
		...
		while( stream.getline( wbuffer, sizeof(wbuffer) ) )
		{
			umtl::static_pool::wstring wfolderInfo = wbuffer;

			umtl::static_pool::wstring wpath = wfolderInfo.substr( 0, wfolderInfo.find( L"\t" ) );

			//bool isDir = wfolderInfo.substr( wfolderInfo.find(L"-")+1, wfolderInfo.find( L"\t" ) ) == std::wstring(L"1") ? true : false;

			struct replace_path { void operator()( wchar_t & c ) { if( c == L'/' ) c=L'\\'; else c = towlower(c); } };

			std::for_each( wpath.begin(), wpath.end(), replace_path() );

			length = WideCharToMultiByte( CP_ACP, 0, wpath.c_str(), wpath.length(), newPath, sizeof(newPath), 0, 0 );

			newPath[length] = 0;

			if( wpath.find( L".ini" ) != std::string::npos )
			{
				ir	= pProcess( newPath, lpOwerClass );
...
const char* Eff2Ut_DxErrMSG(HRESULT hr)
{
	...
	wsprintf( buff, "Error code: 0x%x ====>> DX err msg : %s", hr, DXGetErrorString9( hr ) );
	return buff;
}
