...
BOOL DBWorker::Execute(AgsdQuery* pQuery)
	...
	CHAR* szQuery = pQuery->Get();
	if(!szQuery)
	{
		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "DBWorker::Execute(), pQuery->Get() is NULL. QueryIndex:%d\n", pQuery->m_nIndex);
		AuLogFile_s(RELAY_ERROR_LOG, strCharBuff);
	...
	BOOL bResult = TRUE;
	if (FALSE == m_pStatement->Prepare(szQuery))
		{
		char strCharBuff[256] = { 0, };
	...
	if (FALSE == pQuery->BindParam(m_pStatement))
		{
		pQuery->Dump();

		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "DBWorker::Execute(), m_pStatement->BindParam() returned FALSE. QueryIndex:%d\n", pQuery->m_nIndex);
		AuLogFile_s(RELAY_ERROR_LOG, strCharBuff);
	...
	if (FALSE == m_pStatement->Execute())
	{
		pQuery->Dump();

		char strCharBuff[256] = { 0, };
		sprintf_s(strCharBuff, sizeof(strCharBuff), "DBWorker::Execute(), m_pStatement->Execute() returned FALSE. QueryIndex:%d\n", pQuery->m_nIndex);
		AuLogFile_s(RELAY_ERROR_LOG, strCharBuff);
		if(pQuery->m_pfCallbackFail)
			pQuery->m_pfCallbackFail(NULL, pQuery->m_pClass, pQuery->m_pData);
...
BOOL DBWorker::Connect()
	...
	m_pRowset->InitializeBuffer(16);	// 16K. pre-alloc
