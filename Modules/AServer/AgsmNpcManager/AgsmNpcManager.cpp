...
int AgsmNpcManager::AddExpByPercent(lua_State* L)
{
	...
	INT64 lLevelUpExp = (double)pThis->m_pagpmCharacter->GetLevelUpExp(pcsCharacter);
	INT64 lExp = (double)pThis->m_pagpmCharacter->GetExp(pcsCharacter);
	INT32 nRemainExpPercent = 100 - ( (double)lExp / (double)lLevelUpExp * 100.0);
...
int AgsmNpcManager::AddExp(lua_State* L)
{
	...
	INT32 CID = (INT32)lua_tonumber(L, -2);
	INT32 EXP = (INT64)lua_tonumber(L, -1);
