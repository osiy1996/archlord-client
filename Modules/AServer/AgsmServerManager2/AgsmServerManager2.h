...
class AgsmServerManager2 : public AgsModule
	...
	public:
		// Config DB AIAo A¢´¨¬¢¬
		CHAR				m_szConfigDBDSN[30];
		CHAR				m_szConfigDBUser[30];
		CHAR				m_szConfigDBPwd[30];
#endif
	...
	public:
		...
		//	Stream related
		BOOL		StreamReadServerInfo(CHAR *pszVender, CHAR *pszDatabase, CHAR *pszUser, CHAR *pszPass, CHAR *pszTarget);
		BOOL		StreamWriteServerInfo(CHAR *pszTarget);
		static BOOL	ServerInfoReadCB(PVOID pData, ApModule *pClass, ApModuleStream *pStream);
		static BOOL	ServerInfoWriteCB(PVOID pData, ApModule *pClass, ApModuleStream *pStream);
...
#ifdef _AREA_CHINA_
		//	Config DB AIAo A¢´¨¬¢¬, kelovon, 20051010
		void			SetConfigDBAuthInfo(CHAR *szDSN, CHAR *szUser, CHAR *szPwd);

		BOOL			StreamReadDBServerInfo(CHAR *pszFileName, BOOL bDecryption, CHAR *pszDSN, CHAR *pszUser, CHAR *pszPwd, CHAR *pszVender);

		INT16			AttachServerData(PVOID pClass, INT32 nDataSize, ApModuleDefaultCallBack pfConstructor, ApModuleDefaultCallBack pfDestructor);
#endif
	};
