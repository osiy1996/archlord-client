...
BOOL AgsmItemConvert::CBAdjustSuccessProb(PVOID pData, PVOID pClass, PVOID pCustData)
{
	...
	if (!pcsLuckyScroll || !pcsLuckyScroll->m_pcsItemTemplate)
		return FALSE;
	...
	{
		INT32 lOptionTID = pcsLuckyScroll->m_pcsItemTemplate->m_alOptionTID[i];
		AgpdItemOptionTemplate *pcsItemOptionTemplate = pThis->m_pcsAgpmItem->GetItemOptionTemplate(lOptionTID);
		if(!pcsItemOptionTemplate)
			continue;

		// If Item Have a Option that can be increase probability It's Lucky Scroll
		if(pcsItemOptionTemplate->m_stSkillData.m_lReinforcementRate > 0)
		{
			*pfSuccessProb += (FLOAT)(pcsItemOptionTemplate->m_stSkillData.m_lReinforcementRate / 100.0f);
