...
/* 중국 실명제 관련 콜백함수			                                */
/************************************************************************/
void __stdcall CBGetUserInfo(unsigned int dwRpcId,int nResult, SKeyValue *pKeyValue, int nKeyValueNum);
...
#ifdef _AREA_CHINA_
	AgsmRealNameAuthen		g_smRealNameAuthen;
#endif
#endif
...
#ifdef _AREA_CHINA_
	bool bInitPT = g_csClientInterface.InitPT();
	printf_s("[PT] IsConnect( %d )\n", bInitPT);
...
BOOL AgsmLoginDB::OnInit()
{
	...
	if(FALSE == g_smRealNameAuthen.Init("client.conf"))
		return FALSE;

	if(FALSE == g_smRealNameAuthen.SetCallbackGetUserInfo((SSDOAuthCallBack*)CBGetUserInfo))
		return FALSE;
...
BOOL AgsmLoginDB::ConnectDatabase()
{
	...
	while (NULL != (pAgsdServer = m_pAgsmServerManager->GetGameServers(&nIndex)))
		...
		if (NULL == pAgsmLoginGameDBGroup)
			{
			_tprintf(_T("!!! Error : [%2d][%s] Failed to connect\n"), nIndex, pAgsdServer->m_szDBUser);
			return FALSE;
			}

		// ignore if connected or not
		BOOL bConnect = pAgsmLoginGameDBGroup->Connect(pDatabaseManager,
													   pAgsdServer->m_szDBUser,
													   pAgsdServer->m_szDBPwd,
													   pAgsdServer->m_szDBDSN,
													   nConnect);
...
BOOL AgsmLoginDB::ProcessCheckAccount1(AgsmLoginQueueInfo *pQueryInfo, ApHttp* pHttp)
	...
	else
	{
		...
		AuLogConsole_s("[PT] PutValue(ArchInput, %s)", pQueryInfo->m_szAccountID);
				
		g_csClientInterface.m_pAS->PTAuthen(ptr, pQueryInfo->m_szAccountID, pQueryInfo->m_szPassword);

		AuLogConsole_s("[PT] PTAuthen(%s, %s)", pQueryInfo->m_szAccountID, pQueryInfo->m_szPassword);
...
TCHAR* AgsmLoginDB::GetQuery(TCHAR *pszKey, eAUDB_VENDER eVender)
{
	...
	bool AgsmClientInterface::InitPT()
	{
		...
		printf("[PT] wait for PT connecting...\n");
		for (int i = 0; i < 5; ++i)
		{
			if (m_pAS->IsConnect())
			{
	...
	void AgsmClientInterface::OperateResult(CSessionPtr &p_session, int operation, int result)
	{
		...
		// 실명인증에 실패
		if(FALSE == CheckRealNameAuthen(p_session, tempAccount))
		{
			m_pAgsmLoginClient->SendLoginResult(AGPMLOGIN_RESULT_REAL_NAME_AUTH_FAIL, NULL, nid); 
		}
		else
		{
			// 성공시
			AuLogConsole_s("[PT] %s ( %d )", __FUNCTION__, result);
			switch (result)
			{
			case failed:
			case miss_pwd:
				...
				if (m_pAgsmLoginClient->AddInvalidPasswordLog(nid))
				{
					...
					AuLogConsole_s("AGPMLOGIN_RESULT_INVALID_PASSWORD (%d)", result);
				}
				else
				{
					...
					AuLogConsole_s("AGPMLOGIN_RESULT_INVALID_PASSWORD_LIMIT_EXCEED (%d)", result);
				}
				break;
	...
	BOOL AgsmClientInterface::CheckRealNameAuthen(CSessionPtr& p_session, const CHAR* szAccountName)
	{
		if(NULL == szAccountName)
			return FALSE;

		// Check RealName Authen
		////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

		CHAR* szDigitalAccount	= NULL;
		szDigitalAccount		= const_cast<CHAR*>(p_session->GetValue("DigitId"));
		if(NULL == szDigitalAccount)
			return FALSE;
		
		// 중국 인증도 Blocking으로 처리
		SKeyValue *stSkeyValue[EKEY_VALUE_MAX - 1];
		INT32 lKeyNum = 0;

		if( g_smRealNameAuthen.SyncGetUserInfo(const_cast<CHAR*>(szAccountName), szDigitalAccount, stSkeyValue, &lKeyNum) == FALSE )
		{
			// 실명인증이 실패임
			return FALSE;
		}
...
// 
#ifdef _DEBUG
	#pragma comment(lib,"UserAuthenSDKd.lib")
#else
	#pragma comment(lib,"UserAuthenSDK.lib")
#endif

AgsmRealNameAuthen::AgsmRealNameAuthen()
{
	m_handle = NULL;
...
BOOL AgsmRealNameAuthen::Init(const CHAR* szFileName)
{
	if(NULL == szFileName)
		return FALSE;
	...
	m_handle = CreateSDOAuthBusinessInstance();
	if(NULL == m_handle)
		return FALSE;

	if((lReturn = m_handle->Init(szFileName)) != 0)
		return FALSE;

	return TRUE;
}

BOOL AgsmRealNameAuthen::SetCallbackGetUserInfo(SSDOAuthCallBack *pCallbackFunction)
{
	if(NULL == pCallbackFunction)
		return FALSE;

	if(NULL == m_handle)
		return FALSE;

	m_handle->SetCallBack(pCallbackFunction);

	return TRUE;
}

BOOL AgsmRealNameAuthen::AsyncGetUserInfo(CHAR* strUserID, UINT32 *pdwRpcID)
{
	if(NULL == strUserID)	
		return FALSE;

	INT32 nKeyArray[EKEY_VALUE_MAX - 1];
	nKeyArray[0] = EKEY_VALUE_ISNEEDFULLINFO;
	nKeyArray[1] = EKEY_VALUE_FULLINFO_URL;
	nKeyArray[2] = EKEY_VALUE_FULLINFO_URL2;

	SKeyValue stKeyValue;
	stKeyValue.nKey		= KEY_VALUE_DEFAULT;
	stKeyValue.pValue	= new CHAR[KEY_VALUE_MAX];
	if(stKeyValue.pValue)
	{
		strncpy_s(stKeyValue.pValue, KEY_VALUE_MAX, "", _TRUNCATE);
	}
	
	if(m_handle->AsyncGetUserInfo(USER_TYPE_DIGITAL, strUserID, nKeyArray, EKEY_VALUE_MAX - 1, &stKeyValue, 1, pdwRpcID)==0)
	{
		string strName(strUserID);

		m_mapRPCID.insert(make_pair(*pdwRpcID, strName));
		AuLogConsole_s("AsyncGetUserInfo request success (%s)", strUserID);
	}
	else
	{
		AuLogConsole_s("AsyncGetUserInfo request error (%s)", strUserID);
	}

	if(stKeyValue.pValue)
	{
		delete [] stKeyValue.pValue;
	}	

	return TRUE;
}

BOOL AgsmRealNameAuthen::SyncGetUserInfo(CHAR* strUserID, CHAR* strDigitID, SKeyValue **ppKeyValue, INT32 *pKeyNum)
{
	if(NULL == strUserID)
		return FALSE;
	...
	INT32 nKeyArray[EKEY_VALUE_MAX - 1];
	nKeyArray[EKEY_VALUE_ISNEEDFULLINFO_INDEX] = EKEY_VALUE_ISNEEDFULLINFO;
	nKeyArray[EKEY_VALUE_FULLINFO_URL_INDEX] = EKEY_VALUE_FULLINFO_URL;
	nKeyArray[EKEY_VALUE_FULLINFO_URL2_INDEX] = EKEY_VALUE_FULLINFO_URL2;
	...
	if(stKeyValue.pValue)
	{
		strncpy_s(stKeyValue.pValue, KEY_VALUE_MAX, strUserID, _TRUNCATE);
	}

	INT32 lAuthReturn = 0;

	if((lAuthReturn = m_handle->SyncGetUserInfo(USER_TYPE_DIGITAL, strDigitID, nKeyArray, EKEY_VALUE_MAX - 1, &stKeyValue, 1, ppKeyValue, pKeyNum)) == 0)
	{
		if(ppKeyValue)
		{
			// 실명 인증 성공
			if(ppKeyValue[EKEY_VALUE_ISNEEDFULLINFO_INDEX]->nKey ==  EKEY_VALUE_ISNEEDFULLINFO)
			{
				INT32 lResult = atoi(ppKeyValue[EKEY_VALUE_ISNEEDFULLINFO_INDEX]->pValue);
				if(lResult == 0)
				{
					bEnter = TRUE;
				}
			...
			{
				// 나머지 구현할 내용이 보충되면 보충한다(지금은 13001값밖에 사용안함)
			}
		}

		// 리소스 해제
		m_handle->SyncFreeResult(*ppKeyValue, *pKeyNum);
	}
	else
	{
		bEnter = FALSE;
