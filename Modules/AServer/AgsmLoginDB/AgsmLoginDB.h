...
class AgsmClientInterface : public CClientInterface
{
	...
	public:
		...
		BOOL CheckRealNameAuthen(CSessionPtr& p_session, const CHAR* szAccountName);
...
#define KEY_VALUE_MAX				128
#define KEY_VALUE_DEFAULT			1			// Default임
#define USER_TYPE_DIGITAL			0			// 중국측에서 Digital Account밖에 안쓴다고 한다.
#define USER_TYPE_PT				1			


enum eKeyValue
{
	EKEY_VALUE_PT						=		1,
	EKEY_VALUE_ISNEEDFULLINFO			=		13001,
	EKEY_VALUE_FULLINFO_URL				=		13002,
	EKEY_VALUE_FULLINFO_URL2			=		13003,
	EKEY_VALUE_MAX						=		4
};

enum eKeyValueIndex
{
	EKEY_VALUE_ISNEEDFULLINFO_INDEX		=		0,
	EKEY_VALUE_FULLINFO_URL_INDEX,
	EKEY_VALUE_FULLINFO_URL2_INDEX
};

class AgsmRealNameAuthen
{
private:
	ISDOAuthBusiness * m_handle;

public:
	AgsmRealNameAuthen();
	~AgsmRealNameAuthen();

	typedef map<INT32, string>		RPCIDMAP;

	RPCIDMAP						m_mapRPCID;

	BOOL Init(const CHAR* szFileName);
	BOOL SetCallbackGetUserInfo(SSDOAuthCallBack *pCallbackFunction);

	BOOL AsyncGetUserInfo(CHAR* strUserID, UINT32 *pdwRpcID);
	BOOL SyncGetUserInfo(CHAR* strUserID, CHAR* strDigitID, SKeyValue **ppKeyValue, INT32 *pKeyNum);
};
#endif
...
#ifndef _AREA_CHINA_
		BOOL	ProcessCreateJumpingCharacterCheck(AgsmLoginQueueInfo *pQueryInfo);
#endif
...
#ifdef _AREA_GLOBAL_
		int				GlobalAuthFromDB(AgsmLoginQueueInfo *pQueryInfo);
