...
// 2009.8.04 by supertj
//webzen 게임 인증 로그
//캐릭터가 초기에 게임서버로 진입할때 시간값을 미리 저장해두고
//종료할때 이 함수로 로그인값과 로그아웃값을 한번에 저장한다.
//이로서 쿼리 한번으로 로긴/로그아웃을 기록한다.
BOOL AgpmLog::WriteLogInOut(CHAR* szAccountID,
							const CHAR* szGameID,
							INT32 iServerID/*section*/,
							UINT32 iloginTime, //login or logout time
							UINT32 ilogoutTime, //login or logout time
							CHAR* szCharID,
							UINT32 iCharTID,
							UINT32 iLevel,
							CHAR* szIP)
{
/*
	8, // 게임코드(1 : 뮤, 2 : 썬, 8 : 아크로드, 9 : R2 )
	0, //로그종류(0: 로그인, 1: 로그아웃)
	*/

	AgpdLog_Global cLog;
	_tcsncpy(cLog.m_szAccountID, szAccountID, AGPDLOG_MAX_ACCOUNTID);
	_tcsncpy(cLog.m_szGameID, szGameID, AGPDLOG_MAX_GAMEID);
	cLog.m_lServerID = iServerID;
	cLog.m_uiLoginTime = iloginTime;
	cLog.m_uiLogoutTime = ilogoutTime;
	cLog.m_uiPlayTime = ilogoutTime - iloginTime;
	_tcsncpy(cLog.m_szCharID, szCharID, AGPDLOG_MAX_STR_ID);
	cLog.m_lCharTID = iCharTID;
	cLog.m_lCharLevel = iLevel;
	_tcsncpy(cLog.m_szIP, szIP, 16);
