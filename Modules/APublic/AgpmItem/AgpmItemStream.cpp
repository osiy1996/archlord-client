...
{
#ifdef _AREA_GLOBAL_
	if( !LoadItemTemplateEntryINI( "INI\\ItemTemplateEntry.ini", bDecryption ) )
	{
		OutputDebugString( "Failed to load ItemTemplate Entry File. ( INI\\ItemTemplateEntry.ini )\n" );
		return FALSE;
	}
#endif
...
BOOL AgpmItem::StreamReadOneTemplate( char* pFileName, BOOL bDecryption )
{
	...
	if( !csStream.Open( pFileName, 0, bDecryption ) )
	{
		...
		memset( pBuffer, 0, sizeof( char ) * (nFileSize + 1) );
...
BOOL AgpmItem::StreamReadImportData(CHAR *szFile, CHAR *szDebugString, BOOL bDecryption)
{
	...
	for(INT16 nCol = 1; nCol < pExcel->GetColumn(); ++nCol)
	{
		...
		else if(!strcmp(pszData,	AGPMITEM_IMPORT_ITEM_NPC_ARENA_COIN			)) aColumn[ nCol] = IMVINDEX_ITEM_NPC_ARENA_COIN		;
	...
	for(INT16 nRow = 1; nRow < pExcel->GetRow(); ++nRow)
	{
		...
		for(INT16 nCol = 1; nCol < pExcel->GetColumn(); ++nCol)
		{
			...
			case IMVINDEX_ITEM_PC_PRICE				:
				...
				break;
			case IMVINDEX_ITEM_NPC_ARENA_COIN:
				pszData = pExcel->GetData(nCol, nRow);
				if (pszData)
					pcsAgpdItemTemplate->m_ulNPCArenaCoin = strtoul(
						pszData, NULL, 10);
				break;
