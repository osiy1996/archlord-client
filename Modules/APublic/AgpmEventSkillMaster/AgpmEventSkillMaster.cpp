...
BOOL AgpmEventSkillMaster::OnOperationSkillInitialize(ApdEvent *pcsEvent, AgpdCharacter *pcsCharacter)
{
	...
	while (NULL != token)
	{
		if (strcmp(token, m_pInitString) == 0)
			return FALSE;
...
BOOL AgpmEventSkillMaster::InitializeSkillTree(AgpdCharacter *pcsCharacter, BOOL bAdmin, BOOL bCash)
{
	...
	if(!bAdmin)
	{
		...
		if (g_eServiceArea == AP_SERVICE_AREA_GLOBAL)
		{
			strcat_s(pcsCharacter->m_szSkillInit, AGPACHARACTER_MAX_CHARACTER_SKILLINIT, "initialized");
			strcat_s(pcsCharacter->m_szSkillInit, AGPACHARACTER_MAX_CHARACTER_SKILLINIT, ";");
		}
		else
		{
			strcat_s(pcsCharacter->m_szSkillInit, AGPACHARACTER_MAX_CHARACTER_SKILLINIT, "치약바르자");
			strcat_s(pcsCharacter->m_szSkillInit, AGPACHARACTER_MAX_CHARACTER_SKILLINIT, ";");
		}
	}
	else if(bCash)
