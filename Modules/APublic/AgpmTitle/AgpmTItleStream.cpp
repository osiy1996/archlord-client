...
BOOL AgpmTitle::StreamReadTitleStringTemplate(CHAR* pszFile, BOOL bDecryption)
{
	...
	for(INT32 lRow = 1; lRow < pExcel->GetRow();lRow++)
	{
		...
		CHAR* pszTitleName = pExcel->GetData(AGPMTITLE_TITLE_STRING_EXCEL_COLUMN_TITLE_NAME, lRow);
		strncpy(TitleStringTemplate.m_szTitleName, pszTitleName, AGPDTITLE_MAX_TITLE_NAME_LENTH);

		CHAR* pszTitleNecessaryConditionDesc = pExcel->GetData(AGPMTITLE_TITLE_STRING_EXCEL_COLUMN_TITLE_NECESSARY_CONDITION_DESC, lRow);
		if (pszTitleNecessaryConditionDesc)
			...
			memset(TitleStringTemplate.m_szTitleNecessaryConditionDesc, 0, sizeof(TitleStringTemplate.m_szTitleNecessaryConditionDesc));

		CHAR* pszTitleEffectDesc = pExcel->GetData(AGPMTITLE_TITLE_STRING_EXCEL_COLUMN_TITLE_EFFECT_DESC, lRow);
		if (pszTitleEffectDesc)
			strncpy(TitleStringTemplate.m_szTitleEffectDesc, pszTitleEffectDesc, AGPDTITLE_MAX_TITLE_EFFECT_DESC);
		else
			memset(TitleStringTemplate.m_szTitleEffectDesc, 0, sizeof(TitleStringTemplate.m_szTitleEffectDesc));
		...
		CHAR* pszTitleClass = pExcel->GetData(AGPMTITLE_TITLE_STRING_EXCEL_COLUMN_TITLE_CLASS, lRow);
		strncpy(TitleCategory.m_szTitleClass, pszTitleClass, AGPDTITLE_MAX_TITLE_CLASS_LENTH);

		if(!m_stAgpaTitleCategory.FindByTitleCategoryID(TitleCategory.m_nFirstCategory))
