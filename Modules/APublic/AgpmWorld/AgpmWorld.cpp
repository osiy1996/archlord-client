...
#ifdef _AREA_GLOBAL_
		AUTYPE_CHAR,			AGPDWORLD_MAX_SERVER_REGION,// ServerRegion (Global)
#endif
		AUTYPE_END,				0
		);
}
...
#ifdef _AREA_GLOBAL_
	TCHAR*		szServerRegion = NULL;
	m_csPacket.GetField( TRUE, pvPacket, nSize, &cOperation, &pszWorld, &nStatus, &pszEncodedWorld, &lEncodedWorldLength, &szServerRegion );
#else
...
#ifdef _AREA_GLOBAL_
		return UpdateWorld(pszWorld, nStatus , NULL , 0 , AGPMWORLD_FLAG_NONE , TRUE , szServerRegion ) ? TRUE : FALSE;
#else
...
#ifdef _AREA_GLOBAL_
AgpdWorld* AgpmWorld::UpdateWorld(TCHAR* pszWorld, INT16 nStatus, TCHAR* pszGroup, UINT32 m_ulPriority, UINT32 ulFlag, BOOL bEnum , TCHAR*	szServerRegion )
#else
