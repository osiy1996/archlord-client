...
#ifdef _AREA_GLOBAL_
#define AGPDWORLD_MAX_SERVER_REGION		32
#endif
...
enum eAGPMWORLD_FLAG
{
	...
	AGPMWORLD_FLAG_TEST_SERVER	= 0x10,
};
...
public:
	...
	void	SetTestServer(BOOL bSet)	{	_SetFlag(AGPMWORLD_FLAG_TEST_SERVER, bSet);}
	...
	BOOL	IsTestServer()		{	return _GetFlag(AGPMWORLD_FLAG_TEST_SERVER);}
...
#ifdef _AREA_GLOBAL_
	AgpdWorld*	UpdateWorld(TCHAR* pszWorld, INT16 nStatus, TCHAR* pszGroup = NULL, UINT32 m_ulPriority = 0, UINT32 ulFlag = AGPMWORLD_FLAG_NONE, BOOL bEnum = TRUE , TCHAR* szServerRegion = NULL );	// if world ne then add
#else
