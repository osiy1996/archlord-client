...
BOOL AgpmCashMall::OnReceive(UINT32 ulType, PVOID pvPacket, INT16 nSize, UINT32 ulNID, DispatchArg *pstCheckArg)
{
	...
	if(IsEnableUseCashMall() == FALSE)
	{
		...
		EnumCallback(AGPMCASH_CB_DISABLE_USE_FORAWHILE, pcsCharacter, NULL);
		...
		case AGPMCASH_OPERATION_REQUEST_BUY_ITEM:
			{
				...
				return OnOperationRequestBuyItem(lCID, lProductID, ucMallListVersion, lType);
			}
			break;
...
BOOL AgpmCashMall::OnOperationRequestBuyItem(INT32 lCID, INT32 lProductID, UINT8 ucMallListVersion, INT32 lType)
{
	AgpdCharacter	*pcsCharacter	= m_pcsAgpmCharacter->GetCharacterLock(lCID);
	if (!pcsCharacter)
		return FALSE;
