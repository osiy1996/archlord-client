			...
			if (pszColumnName && pszValue)
			{
				...
				else if (_tcscmp(pszColumnName, AGPMCASHMALL_STREAM_SPECIALFLAG) == 0)
				{
					if (pszValue)
					{
						...
						else if (_tcscmp(pszValue, AGPMCASHMALL_STREAM_FLAG_GPREMIUM) == 0)
							pcsCashMallInfo->m_pstMallItemInfo[i].m_lSpecialFlag	= AGPMCASH_SPECIALFLAG_GPREMIUM;
					}
				}
				...
				#ifdef _AREA_GLOBAL_
				else if (_tcscmp(pszColumnName, AGPMCASHMALL_STREAM_PPCARDONLY) == 0)
				{
					pcsCashMallInfo->m_pstMallItemInfo[i].m_lPPCardOnly = _ttoi(pszValue);
				}
				#endif
			}
		}
