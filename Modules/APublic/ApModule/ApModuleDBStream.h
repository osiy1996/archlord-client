...
#ifndef _APMODULE_DB_STREAM_H_
	#define _APMODULE_DB_STREAM_H_
...
class ApModuleDBStream : public ApModuleStream
	{
	private:
		// ... db
	...
	public:
		ApModuleDBStream();
		virtual ~ApModuleDBStream();

		// open
		BOOL		Open(const CHAR *szTable, INT32 lPartIndex = 0);

		// write method
		BOOL		Write(const CHAR *pszTable, INT32 lPartIndex = 0);	//???

		BOOL		SetSection(CHAR *szSection);
