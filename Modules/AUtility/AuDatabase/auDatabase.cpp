...
//
#ifdef _AREA_CHINA_
HANDLE AuDatabaseBuffer::s_hHeap = NULL;
#endif
...
BOOL AuDatabaseBuffer::Initialize(DWORD dwSize)
	...
	if (NULL != s_hHeap)
		{
		TRACE("!!! Warning : Heap already exist in AuDatabaseBuffer::Initialize()\n");
		return TRUE;
		}
#endif
...
#ifdef _AREA_CHINA_
	s_hHeap = ::HeapCreate(0, 0, dwSize);

	if (NULL == s_hHeap)
		{
		TRACE("!!! Error : Failed to create Heap(Size:%ld) in AuDatabaseBuffer::Initialize()\n", dwSize);
		return FALSE;
		}
#endif
...
void AuDatabaseBuffer::Cleanup()
	{
	#ifdef _AREA_CHINA_
	if (NULL != s_hHeap && s_hHeap != ::GetProcessHeap())
		{
		::HeapDestroy(s_hHeap);
		s_hHeap = NULL;
		}
...
AuDatabaseBuffer::AuDatabaseBuffer()
	...
	if (NULL == s_hHeap)
		{
		s_hHeap = ::GetProcessHeap();	// ... use process' defAult heap.
		TRACE("Info : Use process' default Heap in AuDatabaseBuffer::AuDatabaseBuffer()\n");
		ASSERT (NULL != s_hHeap);
		}
#endif
  	}
...
void AuDatabaseBuffer::Free()
  ...
  #ifdef _DEBUG
	TRACE("Info : Free. Count allocated = [%d]\n", m_dwAllocCount);
  #endif
...
#else
	if (NULL == m_pBuffer)
		m_pBuffer = (LPBYTE)::HeapAlloc(s_hHeap,  HEAP_ZERO_MEMORY, dwSize);
	else
		m_pBuffer = (LPBYTE)::HeapReAlloc(s_hHeap, HEAP_ZERO_MEMORY, pBuffer, m_dwAlloc + dwSize);
#endif
