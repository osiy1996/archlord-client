...
BOOL	AuIniManagerA::ReadFile	( int lIndex, BOOL bDecryption )
{
	...
	//-----------------------------------------------------------------------
	// 새로운 패킹 포멧 사용으로 로딩과정 수정 - kdi 2011.01.27
	...
	if ( GetType() & AUINIMANAGER_TYPE_PART_INDEX )
	{
		...
		/*
		std::string filename = m_strPathName; // 원래 파일이름 보관

		//{@ 파일이름을 암호화 되지 않은상태로 변경
		...
		if( !result )
		{
			...
			result = FromFile(lIndex, bDecryption); // 암호화된 이름으로 로딩
		}

		m_strPathName = filename; // 원래 파일이름으로 되돌림
		*/
	...
	else
	{
		...
		result = FromMemory(bDecryption); // 암호화 되지 않은 이름으로 로딩
		...
		if( !result )
		{
			...
			result = FromMemory( bDecryption ); // 암호화된 이름으로 로딩
		}
