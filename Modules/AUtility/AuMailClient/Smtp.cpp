...
//      pData = (LPBYTE)malloc(dwSize + 1);
      pData = (LPBYTE)new BYTE[dwSize + 1];
...
//    pszBuffer = (LPSTR)malloc(strMsg.length() + 1);
    pszBuffer = (LPSTR)new BYTE[strMsg.length() + 1];
...
//    pszTok1 = (LPTSTR)malloc((dwSize+1) * sizeof(TCHAR));
    pszTok1 = (LPTSTR)new BYTE[(dwSize+1) * sizeof(TCHAR)];
...
//      pszTok1 = (LPTSTR)malloc((lstrlenA(pszTemp)+1) * sizeof(TCHAR));
      pszTok1 = (LPTSTR)new BYTE[(lstrlenA(pszTemp)+1) * sizeof(TCHAR)];
...
//    pszBuff = (LPSTR)malloc(lstrlen(pszCmd)+1);
    pszBuff = (LPSTR)new BYTE[lstrlen(pszCmd)+1];
    _T2A(pszBuff,pszCmd);
