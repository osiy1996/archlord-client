...
{                                                                           \
    ...
    length *= worldData->lightMapDensity *                                  \
            RtAmbOcclMapMaterialGetAmbOcclMapDensityModifier(newPoly.mat);  \
    length *= worldData->lightMapDensity *                                  \
            RtAmbOcclMapMaterialGetAmbOcclMapDensityModifier(newPoly.mat);  \
                                                                            \
...
/*         4. INITIALIZING SLIVER UVs WRONGLY CAN'T BE WORSE THAN LEAVING THEM UNINITIALIZED!               */  \
    ...
    sliver = (length < _rpAmbOcclMapGlobals.sliverAreaThreshold);                                                    \
}                                                                                                               \
MACRO_STOP
...
VertsEqual(RwV3d *vert1, RwV3d *vert2)
{
    ...
    if (RwV3dDotProduct(&delta, &delta) < _rpAmbOcclMapGlobals.vertexWeldThreshold)
    {
        result = TRUE;
    }
                      ...
                      RwTexCoords *texCoords, RwBool posSharing)
{
    ...
    topology->wVerts = (RwUInt32 *) RwMalloc (topology->numVerts * sizeof (RwUInt32),
                                rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FRAME);
                        ...
                        LtMapPolySet **allSets, RwInt32 *numSetsOut)
{
    ...
    while (numProcessedTris < uvObjectData->numTriangles)
    {
        ...
        if (!(RtAmbOcclMapMaterialGetFlags(newPoly.mat) & rtAMBOCCLMAPMATERIALAMBOCCLMAP))
        {
        ...
        *(LtMapPoly *)rwSListGetNewEntry(polySet->members,
        ...
        for (j = 0;j < 3;j++)
        {
           *(RwUInt32 *)rwSListGetNewEntry(edgeList, rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION) =
        ...
        while (rwSListGetNumEntries (edgeList) > 0)
        {
            ...
            while (curEdgeTriList != NULL && curEdgeTriList->list [curEntry] != 0xFFFFFFFF)
            {
                do
                {
                    ...
                    if (RtAmbOcclMapMaterialGetAmbOcclMapDensityModifier (newPoly.mat) !=
                        RtAmbOcclMapMaterialGetAmbOcclMapDensityModifier (polySetMat))
                        break;
                     ...
                     * artifacts */
                    if ((RtAmbOcclMapMaterialGetFlags(newPoly.mat) & rtAMBOCCLMAPMATERIALAMBOCCLMAP) !=
                        (RtAmbOcclMapMaterialGetFlags(polySetMat) & rtAMBOCCLMAPMATERIALAMBOCCLMAP))
                        break;

                    /* Success - Assign the triangle to the polySet */
                    *(LtMapPoly *)rwSListGetNewEntry(polySet->members,
                             rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION) = newPoly;
    ...
    /* Sort the polysets, largest to smallest - the best order for packing */
    {
        ...
        *allSets = (LtMapPolySet *) RwMalloc(numSets * sizeof(LtMapPolySet), rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_EVENT);
        for (i = 0; i < numSets; i++)
        {
            (*allSets) [i] = *(sortSet [i]);
...
//@} Jaewon
{
    ...
    RwTexture *tex = NULL;
    RwUInt32   size;
    RwUInt32   format = RpAmbOcclMapGetRasterFormat();
    ...
    do
    {
		...
		//@} Jaewon
        {
            ...
            if (numSlots != 0 && uvData->slots [numSlots - 1].count == 1)
            {
				...
				//@} Jaewon
				{
					...
					uvData->slots [numSlots - 1].lightMap = NULL;
				}
                _rwSListDestroyEndEntries(rtLtMapGlobals.ltMapSlots, 1);
            }
            else
            {
                size = rpAMBOCCLMAPMINAMBOCCLMAPSIZE;
            }
        }
        else
        {
            ...
            {
                uvObjectData->objectData->lightMapSize = _rpAmbOcclMapGlobals.lightMapSize;
            }
            size = uvObjectData->objectData->lightMapSize;

			//@{ Jaewon 20050107
			// if this is not done, it can fall into an infinite loop.
			while(size*size < (UINT32) numSets*9)
				size *= 2;
			//@} Jaewon
        }
        ...
        if (_rpAmbOcclMapGlobals.renderStyle & rpAMBOCCLMAPSTYLEPOINTSAMPLE)
        {
		...
		//@{ Jaewon 20050105
		// createUVsOnly check added.
		newUseSlot->lightMap = createUVsOnly?NULL:tex;
        newUseSlot->size = size;
        newUseSlot->sampleMap = createUVsOnly?NULL:sampleMap;
...
//@{ Jaewon 20051007
// This code segment is extracted from '_rtLtMapGeometryResetUVs()'
// in order to prevent code duplication.
static void resetUVs(RwTexCoords *texCoords, RwUInt32 numVertices)
{
	RwUInt32     i;

	RWFUNCTION(RWSTRING("resetUVs"));
...
CheckUVsSetUpForLightMapping (const LtMapAllocUVObjectData *uvObjectData)
{
    ...
    if (uvObjectData->geomSource == NULL ||
        RPAMBOCCLMAPGEOMETRYGETDATA (uvObjectData->geomSource)->PolySetArray == NULL)
    {
...
// createUVsOnly parameter added.
static RwBool
LtMapObjectAllocateUV(RtAmbOcclMapLightingSession *session,
                      LtMapAllocUVData       *uvData ,
                      LtMapAllocUVObjectData *uvObjectData,
					  RwBool createUVsOnly)
//@} Jaewon
{
	...
	{
		//@{ Jaewon 20051007
		// Try uv resetting and check resulting uvs again(they may be previously calculated uvs).
		if(createUVsOnly)
			RWRETURN (FALSE);
		else
		{
			resetUVs(uvObjectData->texCoords, (RwUInt32)(uvObjectData->numVertices));
			if(!CheckUVsSetUpForLightMapping(uvObjectData))
				RWRETURN (FALSE);
		}
		//@} Jaewon
    ...
    worldData = RPAMBOCCLMAPWORLDGETDATA(uvData->world);
    if (uvObjectData->geomSource == NULL)
    {
        geometryData = NULL;
    ...
    {
        geometryData = RPAMBOCCLMAPGEOMETRYGETDATA(uvObjectData->geomSource);
	...
	//@{ Jaewon 20050105
	// createUVsOnly
    useSlot = SelectOrCreateLightMap (allSets, numSets, uvData, uvObjectData, createUVsOnly);
	//@} Jaewon
	...
	//@{ Jaewon 20050105
	width  = useSlot->size;//RwRasterGetWidth(RwTextureGetRaster(useSlot->lightMap));
    height = useSlot->size;//RwRasterGetHeight(RwTextureGetRaster(useSlot->lightMap));
	//@} Jaewon
...
//@} Jaewon
{
    ...
    RWASSERT(objectData->flags & rtAMBOCCLMAPOBJECTAMBOCCLMAP);
 ...
 /*       RwDebugSendMessage(rwDEBUGERROR, "LtMapAtomicAllocateUV",
		...
		//@{ Jaewon 20041216
		// fill the second uv set with axis infos.
		RpGeometry *geom2;
		RwV3d *vertices=NULL, *normals=NULL;
		RwTexCoords *texcoords1=NULL, *texcoords2=NULL;
		RwV3d *vertices0=NULL, *normals0=NULL;
		RwTexCoords *texcoords0=NULL;
		RpTriangle *triangles=NULL, *triangles0=NULL;
		RwInt32 numVertices = RpGeometryGetNumVertices(geom);
		RwInt32 nRealloc = 0;

		vertices = (RwV3d *) RwMalloc(2 * RpGeometryGetNumVertices(geom) * sizeof(RwV3d),
                                rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
		normals = (RwV3d *) RwMalloc(2 * RpGeometryGetNumVertices(geom) * sizeof(RwV3d),
                                rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
		texcoords1 = (RwTexCoords *) RwMalloc(2 * RpGeometryGetNumVertices(geom) * sizeof(RwTexCoords),
                                rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
		texcoords2 = (RwTexCoords *) RwMalloc(2 * RpGeometryGetNumVertices(geom) * sizeof(RwTexCoords),
                                rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);

		for(i=0; i < 2*RpGeometryGetNumVertices(geom); ++i)
			texcoords2[i].u = texcoords2[i].v = -1.0f;
		
		RWASSERT(RpGeometryGetNumMorphTargets(geom)==1);
		
		vertices0 = RpMorphTargetGetVertices(RpGeometryGetMorphTarget(geom, 0));
		memcpy(vertices, vertices0, sizeof(RwV3d)*RpGeometryGetNumVertices(geom));
		normals0 = RpMorphTargetGetVertexNormals(RpGeometryGetMorphTarget(geom, 0));
		memcpy(normals, normals0, sizeof(RwV3d)*RpGeometryGetNumVertices(geom));
		texcoords0 = RpGeometryGetVertexTexCoords(geom, 1);
		memcpy(texcoords1, texcoords0, sizeof(RwTexCoords)*RpGeometryGetNumVertices(geom));
		triangles0 = RpGeometryGetTriangles(geom);
		for(i=0; i<RpGeometryGetNumTriangles(geom); ++i)
		{
			RwV3d edge0, edge1, fnorm;
			float axis;
			// get a face normal.
			RwV3dSub(&edge0, &vertices0[triangles0[i].vertIndex[1]], &vertices0[triangles0[i].vertIndex[0]]);
			RwV3dSub(&edge1, &vertices0[triangles0[i].vertIndex[2]], &vertices0[triangles0[i].vertIndex[0]]);
			RwV3dCrossProduct(&fnorm, &edge0, &edge1);
			RwV3dNormalize(&fnorm, &fnorm);
			// decide an axis.
			if(fabs(fnorm.x) > fabs(fnorm.y))
			{
				if(fabs(fnorm.x) > fabs(fnorm.z))
				// x
				{
					if(fnorm.x < 0)
						axis = 0.0f;
			...
			else if(fabs(fnorm.y) > fabs(fnorm.z))
			// y
			{
				if(fnorm.y < 0)
					axis = 0.1f;
			...
			for(j=0; j<3; ++j)
			{
				if(texcoords2[triangles0[i].vertIndex[j]].u > -0.5f && texcoords2[triangles0[i].vertIndex[j]].u != axis)
				// if the axis has been already assigned to a vertex and it's different, duplicate the vertex.
				{
					vertices[numVertices] = vertices0[triangles0[i].vertIndex[j]];
					normals[numVertices] = normals0[triangles0[i].vertIndex[j]];
					texcoords1[numVertices] = texcoords0[triangles0[i].vertIndex[j]];
					
					// adjust vertex indices of the triangle.
					triangles0[i].vertIndex[j] = numVertices;
					++numVertices;
					//@{ Jaewon 20041217
					// realloc
					if(numVertices >= (nRealloc+2)*RpGeometryGetNumVertices(geom))
					{
						++nRealloc;
						vertices = (RwV3d *) RwRealloc(vertices, (nRealloc+2) * RpGeometryGetNumVertices(geom) * sizeof(RwV3d),
												rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
						normals = (RwV3d *) RwRealloc(normals, (nRealloc+2) * RpGeometryGetNumVertices(geom) * sizeof(RwV3d),
												rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
						texcoords1 = (RwTexCoords *) RwRealloc(texcoords1, (nRealloc+2) * RpGeometryGetNumVertices(geom) * sizeof(RwTexCoords),
												rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
						texcoords2 = (RwTexCoords *) RwRealloc(texcoords2, (nRealloc+2) * RpGeometryGetNumVertices(geom) * sizeof(RwTexCoords),
												rwID_AMBOCCLMAPPLUGIN | rwMEMHINTDUR_FUNCTION);
					}
					//@} Jaewon
				}
				texcoords2[triangles0[i].vertIndex[j]].u = axis;
				texcoords2[triangles0[i].vertIndex[j]].v = axis;
			}
		}

		// recreate a geometry with 2 texture coordinate sets.
		geom2 = RpGeometryCreate(numVertices, RpGeometryGetNumTriangles(geom), 
							(RpGeometryGetFlags(geom)&~rpGEOMETRYPRELIT)|rpGEOMETRYTEXCOORDSETS(2));

		// copy data.
		// vertices
		memcpy(RpMorphTargetGetVertices(RpGeometryGetMorphTarget(geom2, 0)), vertices, sizeof(RwV3d)*numVertices);
		// normals
		if(RpMorphTargetGetVertexNormals(RpGeometryGetMorphTarget(geom2, 0)))
		{
			memcpy(RpMorphTargetGetVertexNormals(RpGeometryGetMorphTarget(geom2, 0)), normals, sizeof(RwV3d)*numVertices);
		}
		// texcoords1
		memcpy(RpGeometryGetVertexTexCoords(geom2, 1), texcoords1, sizeof(RwTexCoords)*numVertices);
		// texcoords2
		memcpy(RpGeometryGetVertexTexCoords(geom2, 2), texcoords2, sizeof(RwTexCoords)*numVertices);
		// triangles
		triangles = RpGeometryGetTriangles(geom2);
		for(i=0; i<RpGeometryGetNumTriangles(geom2); ++i)
		{
			RpGeometryTriangleSetVertexIndices(geom2, &triangles[i], triangles0[i].vertIndex[0], triangles0[i].vertIndex[1], triangles0[i].vertIndex[2]);
			RpGeometryTriangleSetMaterial(geom2, &triangles[i], RpGeometryTriangleGetMaterial(geom, &triangles0[i]));
		}
		RpMorphTargetSetBoundingSphere(RpGeometryGetMorphTarget(geom2, 0), RpMorphTargetGetBoundingSphere(RpGeometryGetMorphTarget(geom, 0)));
		RpGeometryUnlock(geom2);

		//@{ Jaewon 20041217
//@{ Jaewon 20051017
// Do not sort-by-material geometry-effect-enabled terrain atomics.
// Otherwise the second texture coordinate set will not sync with vertices in the next run.
#ifndef USE_MFC
		{
			// weird renderware stuff... if this is not done, writing the geometry fails.
			RpGeometry *geomSorted = RpGeometrySortByMaterial(geom2, NULL);
			RpGeometryDestroy(geom2);
			geom2 = geomSorted;
		}
#endif
//@} Jaewon
		...
		{
			RpCollisionGeometryBuildData(geom2, (RpCollisionBuildParam *)NULL);
		}
		//@} Jaewon

		// set the new geometry to the atomic.
		RpAtomicSetGeometry(atomic, geom2, rpATOMICSAMEBOUNDINGSPHERE);

		// delete temporary storages.
		RwFree(vertices);
		RwFree(normals);
		RwFree(texcoords1);
		RwFree(texcoords2);
		...
		geom = geom2;
		uvObjectData.geomSource   = geom2;
		uvObjectData.matList      = &(geom2->matList);
		uvObjectData.numVertices  = RpGeometryGetNumVertices(geom2);
		uvObjectData.vertices     = RpMorphTargetGetVertices(RpGeometryGetMorphTarget(geom2, 0));
		uvObjectData.texCoords    = RpGeometryGetVertexTexCoords(geom2, 2);
		RWASSERT(uvObjectData.texCoords);
	...
	//@{ Jaewon 20050105
	// createUVsOnly
    if (LtMapObjectAllocateUV(session, uvData, &uvObjectData, createUVsOnly) == FALSE)
	//@} Jaewon
        RWRETURN (NULL);
    RWASSERT(NULL != objectData->lightMap);
	...
	for(i=0; i<RpGeometryGetNumMaterials(geom); ++i)
	{
		RpMaterialD3DFxSetEffect(RpGeometryGetMaterial(geom, i), "ambientOcclusion.fx", NULL);

		// let the matd3dfx know its ambient occlusion texture(objectData->lightMap).
		RpMaterialD3DFxSetTexture(RpGeometryGetMaterial(geom, i), "AmbientOcclusionTex", objectData->lightMap);
	}
	}
...
{
    //@{ Jaewon 20051007
	// The core functionality has moved to 'resetUVs()'.
	...
	resetUVs(texCoords, (RwUInt32)RpGeometryGetNumVertices(geom));
