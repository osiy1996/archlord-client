...
void CEffToolApp::OnFileNew() 
{
	...
	else
	{
		...
		{
			LPEFFSET pNewEffSet	=  AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet(dlgNewEffSet.m_dwEffSetID);
			if( !pNewEffSet )
			{
				ErrToWnd( "!pNewEffSEt" );
			}
...
{
	AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_MakeEffFile();
}
...
BOOL CB_NEXT( stAgcmEffectNoticeEffectProcessData stProcessData, PVOID pClass=NULL )
{
	...
	{
		char	szInfo[128]	= "";
		sprintf( szInfo, "%d / %d, id : %d", G_EFFSETQUEUE.size(), AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_GetEffSetMng().size(), useInfo.m_ulEffID );
		::AfxGetMainWnd()->SetWindowText( szInfo );
	}

	//filltering
	LPEFFSET	pEffSet	= AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet( useInfo.m_ulEffID );
	if( !pEffSet													||
...
void CEffToolApp::OnCalcboundAlleff()
{
	...
	LPEffectSetMapCItr	it_curr = AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_GetEffSetMng().begin();
	LPEffectSetMapCItr	it_last = AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_GetEffSetMng().end();

	G_EFFSETQUEUE.resize( AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_GetEffSetMng().size() );
...
void CEffToolApp::bCalcBound(UINT32 ulEffID)
{
	...
	LPEFFSET	pEffSet	= AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet( ulEffID );
	if( !pEffSet )
	{
