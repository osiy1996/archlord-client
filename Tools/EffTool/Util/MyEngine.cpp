...
BOOL MyEngine::OnRegisterModule()
{
	...
	g_pAgcmEff2->SetGlobalVariable( NULL, NULL/*pFrmMainCharac*/, m_pWorld, m_pCamera );
...
//#endif
	...
	g_pAgcmEff2->SetMainCharacterFrame( NULL , CGlobalVar::bGetInst().bGetPtrFrame(CGlobalVar::e_frm_blue) );

	g_pAgcmSound->SetReadFromPack( TRUE );

	g_pAgcmSound->UnMuteSound( SoundType_Effect );
...
VOID MyEngine::bLoadTex(LPCSTR fName, INT* pnWidth, INT* pnHeight, INT* pnDepth, LPCTSTR* szFmt, LPCTSTR szDir)
{
	...
	strcpy( szPath, szDir ? szDir : AgcuEffPath::GetPath_Tex() );
	strcat( szTex, fName );
