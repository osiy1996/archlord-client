...
#include "../XTPPropertyGridItemCustum/XTPGI_Camera.h"
#include "../XTPPropertyGridItemCustum/XTPGI_PostFX.h"
	...
	case XTP_PGN_ITEMVALUE_CHANGED:
		{
			...
			LPEFFSET pEffSet = AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet(m_ulEffSetID );
			if( !pEffSet )
			{
				ErrToWnd("AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet failed!");
				return 0;
			}

			AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl( m_ulEffSetID );
...
INT CDlgGridEffSet::bSetForEffBase(AgcdEffSet* pEffSet, AgcdEffBase* pEffBase)
{
	...
	else if( dynamic_cast<AgcdEffPostFX*>(pEffBase))		pBase->AddChildItem( new CXTPGI_PostFX(strCaption , dynamic_cast<AgcdEffPostFX*>(pEffBase)) )->Expand();
	else if( dynamic_cast<AgcdEffCamera*>(pEffBase))		pBase->AddChildItem( new CXTPGI_Camera(strCaption , dynamic_cast<AgcdEffCamera*>(pEffBase)) )->Expand();
...
INT CDlgGridEffSet::bSetForEffAnim(AgcdEffSet* pEffSet, AgcdEffBase* pEffBase, AgcdEffAnim* pEffAnim)
{
	...
	//case AgcdEffAnim::E_EFFANIM_POSTFX:		pAnim = pCategory->AddChildItem( new CXTPGI_PostFX(strCaption, dynamic_cast<AgcdEffAnim_PostFX*>(pEffAnim)));		break;
	case AgcdEffAnim::E_EFFANIM_CAMERA:		pAnim = pCategory->AddChildItem( new CXTPGI_ACamera(strCaption, dynamic_cast<AgcdEffAnim_Camera*>(pEffAnim)) );		break;
	}
