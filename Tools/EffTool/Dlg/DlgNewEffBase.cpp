...
BEGIN_MESSAGE_MAP(CDlgNewEffBase, CDialog)
	...
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_RADIO_FX, &CDlgNewEffBase::OnBnClickedRadioFx)
	ON_BN_CLICKED(IDOK, &CDlgNewEffBase::OnBnClickedOk)
	ON_BN_CLICKED(IDC_RADIO_CAMERA, &CDlgNewEffBase::OnBnClickedRadioCamera)
END_MESSAGE_MAP()
...
void CDlgNewEffBase::OnOK() 
{
	...
	case AgcdEffBase::E_EFFBASE_OBJECT:
		{
			if( IDYES != AfxMessageBox(_T("dummy object 로 생성하시겠습니까?"), MB_YESNO) )
			{
				...
				strpath += AgcuEffPath::GetPath_Clump();
				strpath += _T("*.rws");
	...
	case AgcdEffBase::E_EFFBASE_POSTFX:
		break;

	case AgcdEffBase::E_EFFBASE_CAMERA:
...
void CDlgNewEffBase::OnBnClickedRadioFx()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
...
void CDlgNewEffBase::OnBnClickedRadioCamera()
{
	UpdateData(TRUE);
...
void CDlgNewEffBase::OnBnClickedOk()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	OnOK();
}
