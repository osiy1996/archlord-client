...
void CDlgListEffSet::OnDblclkList(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	{
		m_pEffSet = AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet( dwr );
		if( m_pEffSet )
...
{
	const LPEffectSetMap& mapEffSet = g_pAgcmEff2->ForTool_GetEffSetMng();
...
INT CDlgListEffSet::bReloadAllItem()
{
	...
	bChk = g_pAgcmEff2->ForTool_ReloadAllEffect();
	ASSERT( bChk );
