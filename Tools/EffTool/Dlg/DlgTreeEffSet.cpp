...
void CDlgTreeEffSet::OnDblclkTree(NMHDR* pNMHDR, LRESULT *pResult)
{
	...
	if( CToolOption::bGetInst().bShowOnlyOneEff() )
	{
		...
		{
			AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl((*it_curr).m_ulID);
		}
	}
	...
	case Lparam::E_LPARAM_ANIM:
		{
			...
			if( IDOK == dlgItem.DoModal() )
			{
				...
				if( pAnimUV && dlgItem.bGetMultUV() )
				{
					...
					{
						int	portion	= (i-tmp.bgn)/tmp.col;
						int rest	= (i-tmp.bgn)%tmp.col;
						float xstep	= 1.f/(float)tmp.col;
						float ystep	= 1.f/(float)tmp.row;
...
void CDlgTreeEffSet::OnMenuItemTrCtrlEdit()
{
	...
	case Lparam::E_LPARAM_EFFSET:
		{
			AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl( pParam->GetEffSet()->bGetID() );
...
{	
	...
	{
		AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl( ulEffSetID );
...
{
	LPEFFSET	pEffSet = AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet(ulEffSetID);
	if( pEffSet )
	{
		if( T_ISMINUS4(AgcdEffGlobal::bGetInst().bGetPtrEffIniMng()->Write_txt( pEffSet ) ))
		{
...
{
	return
	AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_FindEffSet( ulEffSetID );
};
...
INT CDlgTreeEffSet::vDelStEffSet(LPLparam pStLParam)
{
	...
	if( pEffSet )
	{
		if( (*it_f).FlagChk(EffSet::FLAG_NEW) ){
			pEffSet->bRelease();
			AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffSet( ulEffSetID );
		}else{
			AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl( ulEffSetID );
...
INT CDlgTreeEffSet::vDelEffBase(LPLparam pStLParam)
{
	...
	AgcdEffGlobal::bGetInst().bGetPtrAgcmEff2()->ForTool_DeleteEffCtrl( pStLParam->GetEffSet()->bGetID() );
...
INT CDlgTreeEffSet::vSetNewEffBase(AgcdEffSet* pEffSet, AgcdEffBase* pEffBase, CDlgNewEffBase* pDlg)
{
	...
	AgcdEffPostFX*	pEffPostFX	=	dynamic_cast< AgcdEffPostFX* >(pEffBase);

	AgcdEffCamera*	pEffCamera	=	dynamic_cast< AgcdEffCamera* >(pEffBase);


	// pEffPostFX 효과를 적용한다
	if( pEffPostFX )
	{
		//pEffPostFX->bSetSharedParam();
		//pEffPostFX->bSetWaveParam();
		//pEffPostFX->bSetShockwaveParam();
		//pEffPostFX->bSetRippleParam();
