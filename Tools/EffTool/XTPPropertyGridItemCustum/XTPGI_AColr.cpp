...
void CXTPGI_AMissile::OnAddChildItem()
{
	...
	pAnim->AddChildItem(new CXTPGI_Float(_T("Min speed"), &m_pAnimMissile->m_fMinSpeed, m_pAnimMissile->m_fMinSpeed));
	pAnim->AddChildItem(new CXTPGI_Float(_T("Max speed"), &m_pAnimMissile->m_fMaxSpeed, m_pAnimMissile->m_fMaxSpeed));
	...
	pAnim->AddChildItem( new CXTPGI_Float( _T( "Rotate Per Second") , &m_pAnimMissile->m_fRotate , m_pAnimMissile->m_fRotate) );
	pAnim->AddChildItem( new CXTPGI_Float( _T( "Rot,Zigzag Radius") , &m_pAnimMissile->m_fRadius , m_pAnimMissile->m_fRadius) );
	pAnim->AddChildItem( new CXTPGI_Float( _T( "Zigzag Length") , &m_pAnimMissile->m_fZigzagLength , m_pAnimMissile->m_fZigzagLength) );
...
void CXTPGI_ARpSpline::OnAddChildItem()
{
	...
	pAnim->AddChildItem( new CXTPGI_FileName( _T("file") , EFF2_FILE_NAME_MAX
											...
											,AgcuEffPath::GetPath_Anim(), strFilter ) );
	pAnim->Expand();
};
...
//-------------------- CXTPGI_ACamera ----------------------
CXTPGI_ACamera::CXTPGI_ACamera( CString strCaption , AgcdEffAnim_Camera* pEffAnimCamera )
:	CXTPGI_Anim( strCaption , (AgcdEffAnim*)pEffAnimCamera )
, m_pAnimCamera( pEffAnimCamera )
{

}

VOID CXTPGI_ACamera::OnAddChildItem( VOID )
{
	ASSERT(m_pAnimCamera);

	CXTPGI_Anim::OnAddChildItem();

	CXTPPropertyGridItem* pAnim = AddChildItem(new CXTPPropertyGridItem(_T("-- Public --"), _T("category")));
	pAnim->SetReadOnly(TRUE);
	pAnim->AddChildItem(new CXTPGI_Float(_T("Camera Speed"), &m_pAnimCamera->m_fCameraSpeed , m_pAnimCamera->m_fCameraSpeed ));

	pAnim	=	AddChildItem( new CXTPPropertyGridItem( _T("-- Rotate --") , _T("category")));
	pAnim->SetReadOnly(TRUE);
	pAnim->AddChildItem( new CXTPGI_Float( _T("Rotate") , &m_pAnimCamera->m_fCameraRotate , m_pAnimCamera->m_fCameraRotate ));
	pAnim->AddChildItem(new CXTPGI_Float(_T("Repeat Count"), &m_pAnimCamera->m_fRotateCount , m_pAnimCamera->m_fRotateCount ));

	pAnim	=	AddChildItem( new CXTPPropertyGridItem( _T("-- Zoom --") , _T("category")));
	pAnim->SetReadOnly(TRUE);
	pAnim->AddChildItem( new CXTPGI_Float( _T("Length") , &m_pAnimCamera->m_fMoveLength , m_pAnimCamera->m_fMoveLength ));
