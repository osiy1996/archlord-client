...
void CXTPGI_Sound::OnAddChildItem()
{
	...
	m_pItemSoundFile = (CXTPGI_FileName*)AddChildItem(new CXTPGI_FileName(_T("sound"), EFF2_FILE_NAME_MAX, m_pSound->m_szSoundFName, m_pSound->m_szSoundFName, AgcuEffPath::GetPath_Sound(), FILTER ));
	m_pItemSoundFile = (CXTPGI_FileName*)AddChildItem(new CXTPGI_FileName(_T("mono"), EFF2_FILE_NAME_MAX, m_pSound->m_szMonoFName, m_pSound->m_szMonoFName, AgcuEffPath::GetPath_SoundMono(), FILTER ));
