...
MsgProcCallBack				MsgProcCB = NULL;
...
RsSetDebug(void)
{
	const RwChar *buf = RsGlobal.appName;
    RwDebugSetHandler(psDebugMessageHandler);
    RwDebugSendMessage(rwDEBUGMESSAGE, buf,
...
RsRwInitialize(void *displayID)
{
     ...
     */
    //@{ Jaewon 20050615
	// The 3rd parameter added.
	if (!RwEngineInit(&MemoryFunctions, 0, 0, rsRESOURCESDEFAULTARENASIZE))
	//@} Jaewon
    {
        return (FALSE);
    }
...
RsInitialize(void)
{
    ...
    RsGlobal.appName = TEXT("RenderWare Application");
 ...
 *****************************************************************************
 */
void RsSetMsgProcCB(MsgProcCallBack callback)
{
	MsgProcCB = callback;
}
