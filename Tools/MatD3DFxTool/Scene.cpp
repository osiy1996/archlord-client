...
Scene::Scene() : m_pClump(NULL), m_pAnim(NULL), m_pBackTex(NULL)
{
	...
	if(config.is_open())
	{
		...
		char buf[MAX_PATH];
		buf[0] = '\0';
		config >> _modelLoadPath;

		// model save
		buf[0] = '\0';
		config >> _modelSavePath;

		// animation
		buf[0] = '\0';
		config >> _animationPath;
...
Scene::~Scene()
{
	...
	if(config.is_open())
	{
		...
		// save file paths
		// model load
		config << _modelLoadPath << endl;
		// model save
		config << _modelSavePath << endl;
		// animation
		config << _animationPath << endl;
		// texture
		config << _texturePath << endl;
		//@} Jaewon
	...
	if(pSkin)
	{
		...
		//@} Jaewon
		{
			...
			if(doFileDlg(TRUE, TEXT("rws"), pathname, NULL, 
						 ...
						 psWindowGetHandle(),
						 filetitle,
						 TEXT("Open a matching animation"),
						 _animationPath))
			{
				_pThis->loadAnimRWS(pathname);
			}
