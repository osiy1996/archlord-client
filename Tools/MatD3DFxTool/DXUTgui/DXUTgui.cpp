...
#define DXUTGetHWND psWindowGetHandle
#define SPI_GETWHEELSCROLLLINES   104
#define DXUTGetTime 0.001f*(float)RsTimer
...
HRESULT CDXUTDialogResourceManager::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice )
{
    ...
    if( FAILED(hr) )
        return DXUT_ERR( TEXT("D3DXCreateSprite"), hr );
...
HRESULT CDXUTDialogResourceManager::OnResetDevice()
{
    ...
    IDirect3DDevice9* pd3dDevice = ((IDirect3DDevice9*)RwD3D9GetCurrentD3DDevice());//DXUTGetD3DDevice();
...
//        Refresh();
	...
	CDXUTDialogResourceManager* pManager = DXUTGetGlobalDialogResourceManager();
	if( pManager && pManager->m_pSprite )
	{
	    HRESULT hResult = pManager->m_pSprite->Begin( D3DXSPRITE_DONOTSAVESTATE );
		if( hResult == S_OK )
		{
			...
			if( m_bCaption )
			{
				...
				TCHAR wszOutput[256];
				_tcsncpy( wszOutput, m_wszCaption, 256 );
				wszOutput[255] = 0;
				if( m_bMinimized )
					_tcsncat( wszOutput, TEXT(" (Minimized)"), 256 - _tcslen( wszOutput ) );
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddFont( TCHAR* strFaceName, LONG height, LONG weight )
{
    // See if this font already exists
    for( int i=0; i < m_FontCache.GetSize(); i++ )
    {
        DXUTFontNode* pFontNode = m_FontCache.GetAt(i);
        if( 0 == _tcsnicmp( pFontNode->strFace, strFaceName, MAX_PATH-1 ) &&
    ...
    _tcsncpy( pNewFontNode->strFace, strFaceName, MAX_PATH-1 );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetFont( UINT index, TCHAR* strFaceName, LONG height, LONG weight )
{
...
//--------------------------------------------------------------------------------------
int CDXUTDialogResourceManager::AddTexture( TCHAR* strFilename )
{
    // See if this texture already exists
    for( int i=0; i < m_TextureCache.GetSize(); i++ )
    {
        DXUTTextureNode* pTextureNode = m_TextureCache.GetAt(i);
        if( 0 == _tcsnicmp( pTextureNode->strFilename, strFilename, MAX_PATH-1 ) )
        {
            return i;
        }
    }
    ...
    _tcsncpy( pNewTextureNode->strFilename, strFilename, MAX_PATH-1 );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::SetTexture( UINT index, TCHAR* strFilename )
{
    // Make sure the list is at least as large as the index being set
...
bool CDXUTDialog::MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    if( m_bCaption )
    {
        ...
        if( uMsg == WM_LBUTTONDOWN || uMsg == WM_LBUTTONDBLCLK )
        {
            ...
            {
                bDrag = true;
                SetCapture( psWindowGetHandle() );//DXUTGetHWND() );
                return true;
            }
        } else
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddStatic( int ID, const TCHAR* strText, int x, int y, int width, int height, bool bIsDefault, CDXUTStatic** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddButton( int ID, TCHAR* strText, int x, int y, int width, int height, UINT nHotkey, bool bIsDefault, CDXUTButton** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddCheckBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bChecked, UINT nHotkey, bool bIsDefault, CDXUTCheckBox** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddRadioButton( int ID, UINT nButtonGroup, TCHAR* strText, int x, int y, int width, int height, bool bChecked, UINT nHotkey, bool bIsDefault, CDXUTRadioButton** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddEditBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bIsDefault, CDXUTEditBox** ppCreated )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::AddIMEEditBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bIsDefault, CDXUTIMEEditBox** ppCreated )
{
    HRESULT hr = S_OK;
...
HRESULT CDXUTDialog::AddControl( CDXUTControl* pControl )
{
    ...
    if( FAILED(hr) )
        return DXTRACE_ERR( TEXT("CDXUTDialog::InitControl"), hr );

    // Add to the list
    hr = m_Controls.Add( pControl );
    if( FAILED(hr) )
    {
        return DXTRACE_ERR( TEXT("CGrowableArray::Add"), hr );
    }
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::CalcTextRect( const TCHAR* strText, CDXUTElement* pElement, RECT* prcDest, int nCount )
{
    HRESULT hr = S_OK;
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTDialog::DrawText( const TCHAR* strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow, int nCount )
{
    HRESULT hr = S_OK;
...
void CDXUTDialog::OnCycleFocus( bool bForward )
{
    ...
    DXTRACE_ERR( TEXT("CDXUTDialog: Multiple dialogs are improperly chained together"), E_FAIL );
}
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
    ...
    if( FAILED(hr) )
    {
        return DXTRACE_ERR( TEXT("DXUTFindDXSDKMediaFileCch"), hr );
    }
...
{
    SetTexture( 0, TEXT("DXUTControls.dds") );
...
//--------------------------------------------------------------------------------------
D3DCOLOR CDXUTControl::GetTextColor( )
{
    CDXUTElement* pElement = m_Elements.GetAt( 0 );

    if( pElement )
        return pElement->FontColor.States[DXUT_STATE_NORMAL];
	else
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTStatic::GetTextCopy( TCHAR* strDest, UINT bufferCount )
{
    ...
    _tcsncpy( strDest, m_strText, bufferCount );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTStatic::SetText( const TCHAR* strText )
{
    if( strText == NULL )
    {
        m_strText[0] = 0;
	...
	//@{ Jaewon 20041103
	// MAX_PATH -> 10240
    _tcsncpy( m_strText, strText, 10240-1 ); 
	//@} Jaewon
    return S_OK;
}
...
HRESULT CDXUTComboBox::AddItem( const TCHAR* strText, void* pData )
{
    ...
    if( pItem == NULL )
    {
        return DXTRACE_ERR_MSGBOX( TEXT("new"), E_OUTOFMEMORY );
    }
    
    ZeroMemory( pItem, sizeof(DXUTComboBoxItem) );
    _tcsncpy( pItem->strText, strText, 255 );
    pItem->pData = pData;
...
int CDXUTComboBox::FindItem( const TCHAR* strText, UINT iStart )
{
    ...
    for( int i = iStart; i < m_Items.GetSize(); i++ )
    {
        ...
        if( 0 == _tcscmp( pItem->strText, strText ) )
        {
            return i;
        }
    }
...
void* CDXUTComboBox::GetItemData( const TCHAR* strText )
{
    ...
    if( pItem == NULL )
    {
        DXTRACE_ERR( TEXT("CGrowableArray::GetAt"), E_FAIL );
        return NULL;
    }
...
HRESULT CDXUTListBox::AddItem( const TCHAR *wszText, void *pData )
{
    ...
    _tcsncpy( pNewItem->strText, wszText, 256 );
    pNewItem->strText[255] = TEXT('\0');
...
//--------------------------------------------------------------------------------------
void CDXUTEditBox::SetText( TCHAR* wszText, bool bSelected )
{
    assert( wszText != NULL );
...
//--------------------------------------------------------------------------------------
HRESULT CDXUTEditBox::GetTextCopy( TCHAR* strDest, UINT bufferCount )
{
    assert( strDest );

    _tcsncpy( strDest, m_Buffer.GetBuffer(), bufferCount );
    *(strDest + bufferCount - 1) = TEXT('\0');
    ...
    if( m_nCaret != m_nSelStart && OpenClipboard( NULL ) )
    {
        ...
        if( hBlock )
        {
            ...
            if( pwszText )
            {
                ...
                pwszText[nLast - nFirst] = TEXT('\0');  // Terminate it
                GlobalUnlock( hBlock );
            }
        ...
        if( handle )
        {
            ...
            if( pwszText )
            {
                ...
                if( m_Buffer.InsertString( m_nCaret, pwszText ) )
                    PlaceCaret( m_nCaret + _tcslen( pwszText ) );
                m_nSelStart = m_nCaret;
                GlobalUnlock( handle );
            }
        }
...
#define IN_FLOAT_CHARSET( c ) \
    ( (c) == TEXT('-') || (c) == TEXT('.') || ( (c) >= TEXT('0') && (c) <= TEXT('9') ) )
...
void CDXUTEditBox::ParseFloatArray( float *pNumbers, int nCount )
{
    ...
    while( nWritten < nCount && *pToken != TEXT('\0') )
    {
        // Skip leading spaces
        while( *pToken == TEXT(' ') )
            ++pToken;
...
{
    TCHAR wszBuffer[512];
    TCHAR *pNext = wszBuffer;

    for( int i = 0; i < nCount; ++i )
    {
        pNext += _stprintf_s( pNext, 512 - (pNext - wszBuffer), TEXT("%.4f "), pNumbers[i] );
    }
...
UINT (WINAPI * CDXUTIMEEditBox::_GetReadingString)( HIMC, UINT, TCHAR*, PINT, BOOL*, PUINT )
...
TCHAR*    CDXUTIMEEditBox::s_wszCurrIndicator   // Points to an indicator string that corresponds to current input locale
...
HRESULT CDXUTIMEEditBox::StaticOnCreateDevice()
{
    ...
    s_hImcDef = _ImmGetContext( psWindowGetHandle() );//DXUTGetHWND() );
    _ImmReleaseContext( psWindowGetHandle()/*DXUTGetHWND()*/, s_hImcDef );
...
void CDXUTIMEEditBox::CheckToggleState()
{
    ...
    HIMC hImc;
    if( NULL != ( hImc = _ImmGetContext( psWindowGetHandle()/*DXUTGetHWND()*/ ) ) )
    {
        if( s_bChineseIME )
        {
        ...
        _ImmReleaseContext( psWindowGetHandle()/*DXUTGetHWND()*/, hImc );
    }
    else
...
void CDXUTIMEEditBox::SetupImeApi()
{
    ...
    if ( !s_hDllIme )
        return;
    _GetReadingString = (UINT (WINAPI*)(HIMC, UINT, TCHAR*, PINT, BOOL*, PUINT))
...
void CDXUTIMEEditBox::TruncateCompString( bool bUseBackSpace, int iNewStrLen )
{
	...
	int	i;
    int cc = (int) _tcslen( s_CompString.GetBuffer() );
    // Send right arrow keystrokes to move the caret
    //   to the end of the composition string.
    for (i = 0; i < cc - s_nCompCaret; ++i )
        SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYDOWN, VK_RIGHT, 0 );
    SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYUP, VK_RIGHT, 0 );
    ...
    if( iNewStrLen < cc )
    {
        ...
        {
            SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYDOWN, VK_BACK, 0 );  // Backspace character
            SendMessageW( psWindowGetHandle()/*DXUTGetHWND()*/, WM_CHAR, VK_BACK, 0 );
        }
        SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYUP, VK_BACK, 0 );
    }
    else
        iNewStrLen = cc;
    ...
    for (i = 0; i < iNewStrLen; ++i )
        SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYDOWN, VK_LEFT, 0 );
    SendMessage( psWindowGetHandle()/*DXUTGetHWND()*/, WM_KEYUP, VK_LEFT, 0 );
}
...
void CDXUTIMEEditBox::FinalizeString( bool bSend )
{
    ...
    {
        DXUTTRACE( TEXT("CDXUTIMEEditBox::FinalizeString: Reentrant detected!\n") );
        _ImmReleaseContext( psWindowGetHandle()/*DXUTGetHWND()*/, hImc );
        return;
    }
    bProcessing = true;
...
void CDXUTIMEEditBox::GetReadingWindowOrientation( DWORD dwId )
{
    ...
    if( !s_bHorizontalReading && ( dwId & 0x0000FFFF ) == LANG_CHT )
    {
        ...
        _tcscpy( wszRegPath, TEXT("software\\microsoft\\windows\\currentversion\\") );
        _tcscat( wszRegPath, ( dwVer >= MAKEIMEVERSION( 5, 1 ) ) ? TEXT("MSTCIPH") : TEXT("TINTLGNT") );
        LONG lRc = RegOpenKeyEx( HKEY_CURRENT_USER, wszRegPath, 0, KEY_READ, &hKey );
        if (lRc == ERROR_SUCCESS)
        {
            DWORD dwSize = sizeof(DWORD), dwMapping, dwType;
            lRc = RegQueryValueEx( hKey, TEXT("Keyboard Mapping"), NULL, &dwType, (PBYTE)&dwMapping, &dwSize );
            if (lRc == ERROR_SUCCESS)
            {
...
void CDXUTIMEEditBox::GetPrivateReadingString()
{
    ...
    HIMC hImc;
    hImc = _ImmGetContext( psWindowGetHandle()/*DXUTGetHWND()*/ );
    if( !hImc )
    {
        s_bShowReadingWindow = false;
        return;
    ...
    DWORD dwErr = 0;
    TCHAR *pwszReadingStringBuffer = NULL;  // Buffer for when the IME supports GetReadingString()
    TCHAR *wstr = 0;
    ...
    if( _GetReadingString )
    {
        ...
        if( dwReadingStrLen )
        {
            wstr = pwszReadingStringBuffer = (TCHAR*)HeapAlloc( GetProcessHeap(), 0, sizeof(TCHAR) * dwReadingStrLen );
    ...
    else
    {
        ...
        switch( dwId )
        {
            ...
            case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
            {
                ...
                int nTcharSize = ( osi.dwPlatformId == VER_PLATFORM_WIN32_NT ) ? sizeof(TCHAR) : sizeof(char);
    ...
    else
    {
        ...
        for( i = 0, j = 0; i < dwReadingStrLen; ++i, ++j ) // dwlen > 0, if known IME
        {
            ...
            {
                uCodePage = _tcstoul( wszCodePage, NULL, 0 );
            }
    ...
    if( s_bHorizontalReading )
    {
        ...
        for( UINT i = 0; i < s_CandList.dwCount; ++i )
        {
            ...
            _tcsncat( s_wszReadingString, s_CandList.awszCandidate[i], 32 - _tcslen( s_wszReadingString ) - 1 );
        }
    }
...
void CDXUTIMEEditBox::OnFocusIn()
{
    ...
    {
        _ImmAssociateContext( psWindowGetHandle()/*DXUTGetHWND()*/, s_hImcDef );
        CheckToggleState();
    } else
...
bool CDXUTIMEEditBox::StaticMsgProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    ...
    switch( uMsg )
    {
            ...
            if( wParam )
            {
                ...
                if( phKL )
                {
                    ...
                    for( UINT i = 0; i < cKL; ++i )
                    {
                        ...
                        _tcsncpy( Locale.m_wszLang, wszDesc, sizeof(Locale.m_wszLang) / sizeof(Locale.m_wszLang[0]) );
                        Locale.m_wszLang[sizeof(Locale.m_wszLang) / sizeof(Locale.m_wszLang[0]) - 1] = TEXT('\0');
        ...
        case WM_INPUTLANGCHANGE:
            DXUTTRACE( TEXT("WM_INPUTLANGCHANGE\n") );
            {
        ...
        case WM_IME_SETCONTEXT:
            DXUTTRACE( TEXT("WM_IME_SETCONTEXT\n") );
            //
        ...
        case WM_IME_STARTCOMPOSITION:
            DXUTTRACE( TEXT("WM_IME_STARTCOMPOSITION\n") );
            ResetCompositionString();
        ...
        case WM_IME_COMPOSITION:
            DXUTTRACE( TEXT("WM_IME_COMPOSITION\n") );
            return true;
    }
            ...
            if( s_CandList.bShowWindow && PtInRect( &s_CandList.rcCandidate, pt ) )
            {
                ...
                } else
                {
                    ...
                    for( UINT i = 0; i < s_CandList.dwCount; ++i )
                    {
                        ...
                        {
                            // Haven't found it.
                            nEntryStart += _tcslen( s_CandList.awszCandidate[i] ) + 1;  // plus space separator
                        } else
                        {
    ...
    *trapped = false;
    ...
    switch( uMsg )
    {
        ...
        case WM_IME_ENDCOMPOSITION:
            DXUTTRACE( TEXT("WM_IME_ENDCOMPOSITION\n") );
            TruncateCompString();
            ResetCompositionString();
        ...
        case WM_IME_NOTIFY:
            DXUTTRACE( TEXT("WM_IME_NOTIFY %u\n"), wParam );
            switch( wParam )
            {
                case IMN_SETCONVERSIONMODE:
                    DXUTTRACE( TEXT("  IMN_SETCONVERSIONMODE\n") );
                case IMN_SETOPENSTATUS:
                    DXUTTRACE( TEXT("  IMN_SETOPENSTATUS\n") );
                    CheckToggleState();
                    break;
                ...
                {
                    DXUTTRACE( wParam == IMN_CHANGECANDIDATE ? TEXT("  IMN_CHANGECANDIDATE\n") : TEXT("  IMN_OPENCANDIDATE\n") );
                    ...
                    *trapped = true;
                    ...
                    if( lpCandList )
                    {
                            ...
                            i++, j++ )
                        {
                            ...
                            TCHAR* pwsz = s_CandList.awszCandidate[j];
                            // For every candidate string entry,
                            // write [index] + Space + [String] if vertical,
                            // write [index] + [String] + Space if horizontal.
	                        *pwsz++ = (TCHAR)( TEXT('0') + ( (j + 1) % 10 ) );  // Index displayed is 1 based
	                        if( s_bVerticalCand )
		                        *pwsz++ = TEXT(' ');
                            TCHAR *pwszNewCand = (TCHAR*)( (LPBYTE)lpCandList + lpCandList->dwOffset[i] );
	                        while ( *pwszNewCand )
		                        *pwsz++ = *pwszNewCand++;
	                        if( !s_bVerticalCand )
		                        *pwsz++ = TEXT(' ');
                        ...
                        if( !s_bVerticalCand )
                        {
                            TCHAR wszCand[256] = TEXT("");

                            s_CandList.nFirstSelected = 0;
                            s_CandList.nHoriSelectedLen = 0;
                            for( UINT i = 0; i < MAX_CANDLIST; ++i )
                            {
                                if( s_CandList.awszCandidate[i][0] == TEXT('\0') )
                                    break;

                                TCHAR wszEntry[32];
                                _stprintf( wszEntry, TEXT("%s "), s_CandList.awszCandidate[i] );
                            ...
                            wszCand[_tcslen(wszCand) - 1] = TEXT('\0');  // Remove the last space
                ...
                {
                    DXUTTRACE( TEXT("  IMN_CLOSECANDIDATE\n") );
...
void CDXUTEditBox::CUniBuffer::InitializeUniscribe()
{
    ...
    if( !::GetSystemDirectory( wszPath, MAX_PATH+1 ) )
        return;
    _tcscat( wszPath, UNISCRIBE_DLLNAME );
...
bool CDXUTEditBox::CUniBuffer::Grow( int nNewSize )
{
    ...
    if( !pTempBuffer )
        return false;
    if( m_pwszBuffer )
        CopyMemory( pTempBuffer, m_pwszBuffer, (m_nTextSize + 1) * sizeof(TCHAR) );
    delete[] m_pwszBuffer;
...
{
    m_pwszBuffer = new TCHAR[nInitialSize];
    *m_pwszBuffer = 0;
...
//--------------------------------------------------------------------------------------
TCHAR& CDXUTEditBox::CUniBuffer::operator[]( int n )  // No param checking
{
...
//--------------------------------------------------------------------------------------
bool CDXUTEditBox::CUniBuffer::InsertChar( int nIndex, TCHAR wChar )
{
    assert( nIndex >= 0 );
    ...
    MoveMemory( m_pwszBuffer + nIndex + 1, m_pwszBuffer + nIndex, sizeof(TCHAR) * ( m_nTextSize - nIndex + 1 ) );
...
bool CDXUTEditBox::CUniBuffer::RemoveChar( int nIndex )
{
    ...
    MoveMemory( m_pwszBuffer + nIndex, m_pwszBuffer + nIndex + 1, sizeof(TCHAR) * ( m_nTextSize - nIndex ) );
    --m_nTextSize;
    m_bAnalyseRequired = true;
    return true;
}
...
// If nIndex == -1, insert to the end.
//--------------------------------------------------------------------------------------
bool CDXUTEditBox::CUniBuffer::InsertString( int nIndex, const TCHAR *pStr, int nCount )
{
    assert( nIndex >= 0 );
    ...
    MoveMemory( m_pwszBuffer + nIndex + nCount, m_pwszBuffer + nIndex, sizeof(TCHAR) * ( m_nTextSize - nIndex + 1 ) );
...
//--------------------------------------------------------------------------------------
bool CDXUTEditBox::CUniBuffer::SetText( TCHAR* wszText )
{
    assert( wszText != NULL );

    int nRequired = int(_tcslen( wszText ) + 1);
