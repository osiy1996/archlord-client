...
#define DXUTTrace(a, b, c, d, e)           DXTrace(a, b, c, d, e)

#if defined(DEBUG) | defined(_DEBUG)
    #define V(x)           { hr = x; if( FAILED(hr) ) { DXUTTrace( __FILE__, (DWORD)__LINE__, hr, TEXT(#x), true ); } }
    #define V_RETURN(x)    { hr = x; if( FAILED(hr) ) { return DXUTTrace( __FILE__, (DWORD)__LINE__, hr, TEXT(#x), true ); } }
#else
    #define V(x)           { hr = x; }
...
//@{ Jaewon 20041104
#define DXUTTRACE                  DXUTOutputDebugString//(__noop)
//@} Jaewon
#endif
...
{
public:
    CDXUTPerfEventGenerator( D3DCOLOR color, TCHAR* pstrMessage ) { DXUT_BeginPerfEvent( color, pstrMessage ); }
