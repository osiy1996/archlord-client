...
//--------------------------------------------------------------------------------------
bool DXUTFindMediaSearchTypicalDirs( TCHAR* strSearchPath, int cchSearch, TCHAR* strLeaf, TCHAR* strExePath, TCHAR* strExeName, TCHAR* strMediaDir );
...
//--------------------------------------------------------------------------------------
HRESULT DXUTGetDXSDKMediaPathCch( TCHAR* strDest, int cchDest )
{
    ...
    HKEY  hKey;
    LONG lResult = RegOpenKeyEx( HKEY_LOCAL_MACHINE, TEXT("Software\\Microsoft\\DirectX SDK"), 0, KEY_READ, &hKey );
    if( ERROR_SUCCESS != lResult ) 
        return E_FAIL;

    DWORD dwType;
    DWORD dwSize = cchDest * sizeof(TCHAR);
    lResult = RegQueryValueEx( hKey, TEXT("DX9S4SDK Samples Path"), NULL, &dwType, (BYTE*)strDest, &dwSize );
...
//       pass in sizeof(strDest) on UNICODE builds.
//--------------------------------------------------------------------------------------
HRESULT DXUTFindDXSDKMediaFileCch( TCHAR* strDestPath, int cchDest, TCHAR* strFilename )
{
    bool bFound;
    ...
    strLastSlash = _tcschr( strExePath, TEXT('\\') );
    if( strLastSlash )
    {
        ...
        *strLastSlash = 0;
        ...
        strLastSlash = _tcschr( strExeName, TEXT('.') );
        if( strLastSlash )
            *strLastSlash = 0;
    }
    ...
    _stprintf_s( strSearchFor, MAX_PATH, TEXT("media\\%s"), strFilename ); strSearchFor[MAX_PATH-1] = 0;
    ...
    bFound = DXUTFindMediaSearchParentDirs( strDestPath, cchDest, TEXT("."), strLeafName );
    if( bFound )
        return S_OK;
    ...
    _stprintf_s( strLeafName, MAX_PATH, TEXT("media\\%s"), strFilename ); strLeafName[MAX_PATH-1] = 0;
    bFound = DXUTFindMediaSearchParentDirs( strDestPath, cchDest, TEXT("."), strLeafName );
    if( bFound )
        return S_OK;
...
//--------------------------------------------------------------------------------------
bool DXUTFindMediaSearchTypicalDirs( TCHAR* strSearchPath, int cchSearch, TCHAR* strLeaf, 
                                     TCHAR* strExePath, TCHAR* strExeName, TCHAR* strMediaDir )
{
    ...
    // Search in ..\  
    _stprintf_s( strSearchPath, cchSearch, TEXT("..\\%s"), strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    _stprintf_s( strSearchPath, cchSearch, TEXT("%s\\%s"), strExePath, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    _stprintf_s( strSearchPath, cchSearch, TEXT("%s\\..\\%s\\%s"), strExePath, strExeName, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
    ...
    _stprintf_s( strSearchPath, cchSearch, TEXT("%s%s"), strMediaDir, strLeaf ); strSearchPath[cchSearch-1] = 0;
    if( GetFileAttributes( strSearchPath ) != 0xFFFFFFFF )
        return true;
...
bool DXUTFindMediaSearchParentDirs( TCHAR* strSearchPath, int cchSearch, TCHAR* strStartAt, TCHAR* strLeafName )
{
    ...
    {
        _stprintf_s( strFullFileName, MAX_PATH, TEXT("%s\\%s"), strFullPath, strLeafName ); strFullFileName[MAX_PATH-1] = 0;       
        ...
        _stprintf_s( strSearch, MAX_PATH, TEXT("%s\\.."), strFullPath ); strSearch[MAX_PATH-1] = 0;
...
HRESULT CDXUTResourceCache::CreateTextureFromFileEx( LPDIRECT3DDEVICE9 pDevice, LPCTSTR pSrcFile, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE9 *ppTexture )
{
    ...
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        if( Entry.Location == DXUTCACHE_LOCATION_FILE &&
            !_tcscmp( Entry.wszSource, pSrcFile ) &&
            Entry.Width == Width &&
            Entry.Height == Height &&
    ...
    _tcscpy( NewEntry.wszSource, pSrcFile );
...
HRESULT CDXUTResourceCache::CreateTextureFromResourceEx( LPDIRECT3DDEVICE9 pDevice, HMODULE hSrcModule, LPCTSTR pSrcResource, UINT Width, UINT Height, UINT MipLevels, DWORD Usage, D3DFORMAT Format, D3DPOOL Pool, DWORD Filter, DWORD MipFilter, D3DCOLOR ColorKey, D3DXIMAGE_INFO *pSrcInfo, PALETTEENTRY *pPalette, LPDIRECT3DTEXTURE9 *ppTexture )
{
    ...
    for( int i = 0; i < m_TextureCache.GetSize(); ++i )
    {
        ...
        if( Entry.Location == DXUTCACHE_LOCATION_RESOURCE &&
            ...
            !_tcscmp( Entry.wszSource, pSrcResource ) &&
            Entry.Width == Width &&
            Entry.Height == Height &&
            Entry.MipLevels == MipLevels &&
            Entry.Usage == Usage &&
    ...
    _tcscpy( NewEntry.wszSource, pSrcResource );
    NewEntry.Width = Width;
    NewEntry.Height = Height;
    NewEntry.MipLevels = MipLevels;
    NewEntry.Usage = Usage;
...
HRESULT CDXUTResourceCache::CreateFont( LPDIRECT3DDEVICE9 pDevice, UINT Height, UINT Width, UINT Weight, UINT MipLevels, BOOL Italic, DWORD CharSet, DWORD OutputPrecision, DWORD Quality, DWORD PitchAndFamily, LPCTSTR pFacename, LPD3DXFONT *ppFont )
{
    ...
    _tcsncpy( Desc.FaceName, pFacename, LF_FACESIZE );
    Desc.FaceName[LF_FACESIZE - 1] = TEXT('\0');
...
VOID CFirstPersonCamera::FrameMove( FLOAT fElapsedTime )
{
    if( DXUTGetGlobalTimer()->IsStopped() )         
        fElapsedTime = 1.0f / 100.0f;//DXUTGetFPS();
...
//--------------------------------------------------------------------------------------
TCHAR* DXUTD3DFormatToString( D3DFORMAT format, bool bWithPrefix )
{
    TCHAR* pstr = NULL;
    switch( format )
    {
    ...
    if( bWithPrefix || _tcsstr( pstr, TEXT("D3DFMT_") )== NULL )
        ...
        return pstr + _tcslen( TEXT("D3DFMT_") );
}
...
//       argument list.
//--------------------------------------------------------------------------------------
VOID DXUTOutputDebugString( TCHAR* strMsg, ... )
{
    ...
    _stprintf_s( strBuffer, 512, strMsg, args );
    strBuffer[511] = TEXT('\0');
    va_end(args);
...
{
    if( NULL == m_pFont ) 
        return DXUT_ERR_MSGBOX( TEXT("DrawTextLine"), E_INVALIDARG );
    ...
    if( FAILED(hr) )
        return DXTRACE_ERR_MSGBOX( TEXT("DrawText"), hr );
...
HRESULT CDXUTDirectionWidget::StaticOnCreateDevice( IDirect3DDevice9* pd3dDevice )
{
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, TEXT("UI\\DXUTShared.fx" ) ) );
    ...
    V_RETURN( DXUTFindDXSDKMediaFileCch( str, MAX_PATH, TEXT("UI\\arrow.x" ) ) );
...
typedef INT         (WINAPI * LPD3DPERF_BEGINEVENT)(D3DCOLOR, TCHAR*);
typedef INT         (WINAPI * LPD3DPERF_ENDEVENT)(void);
typedef VOID        (WINAPI * LPD3DPERF_SETMARKER)(D3DCOLOR, TCHAR*);
typedef VOID        (WINAPI * LPD3DPERF_SETREGION)(D3DCOLOR, TCHAR*);
...
int WINAPI DXUT_Dynamic_D3DPERF_BeginEvent( D3DCOLOR col, TCHAR* wszName )
{
...
void WINAPI DXUT_Dynamic_D3DPERF_SetMarker( D3DCOLOR col, TCHAR* wszName )
{
    if( DXUT_EnsureD3DAPIs() && s_DynamicD3DPERF_SetMarker != NULL )
        s_DynamicD3DPERF_SetMarker( col, wszName );
}

void WINAPI DXUT_Dynamic_D3DPERF_SetRegion( D3DCOLOR col, TCHAR* wszName )
{
