...
public:
    ...
    // Control creation
    HRESULT AddStatic( int ID, const TCHAR* strText, int x, int y, int width, int height, bool bIsDefault=false, CDXUTStatic** ppCreated=NULL );
    HRESULT AddButton( int ID, TCHAR* strText, int x, int y, int width, int height, UINT nHotkey=0, bool bIsDefault=false, CDXUTButton** ppCreated=NULL );
    HRESULT AddCheckBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bChecked=false, UINT nHotkey=0, bool bIsDefault=false, CDXUTCheckBox** ppCreated=NULL );
    HRESULT AddRadioButton( int ID, UINT nButtonGroup, TCHAR* strText, int x, int y, int width, int height, bool bChecked=false, UINT nHotkey=0, bool bIsDefault=false, CDXUTRadioButton** ppCreated=NULL );
    ...
    HRESULT AddEditBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bIsDefault=false, CDXUTEditBox** ppCreated=NULL );
    HRESULT AddIMEEditBox( int ID, TCHAR* strText, int x, int y, int width, int height, bool bIsDefault=false, CDXUTIMEEditBox** ppCreated=NULL );
    ...
    HRESULT CalcTextRect( const TCHAR* strText, CDXUTElement* pElement, RECT* prcDest, int nCount = -1 );
    HRESULT DrawText( const TCHAR* strText, CDXUTElement* pElement, RECT* prcDest, bool bShadow = false, int nCount = -1 );
    ...
    void SetCaptionText( const TCHAR *pwszText ) { _tcsncpy( m_wszCaption, pwszText, sizeof(m_wszCaption)/sizeof(m_wszCaption[0]) ); m_wszCaption[sizeof(m_wszCaption)/sizeof(m_wszCaption[0])-1] = 0; }
    ...
    // all the controls.
    HRESULT       SetFont( UINT index, TCHAR* strFaceName, LONG height, LONG weight );
    DXUTFontNode* GetFont( UINT index );

    HRESULT          SetTexture( UINT index, TCHAR* strFilename );
...
{
public:
    int AddFont( TCHAR* strFaceName, LONG height, LONG weight );
    int AddTexture( TCHAR* strFilename );
...
public:
    ...
    HRESULT GetTextCopy( TCHAR* strDest, UINT bufferCount );
    TCHAR* GetText() { return m_strText; }
    HRESULT SetText( const TCHAR* strText );


protected:
	//@{ Jaewon 20041103
	// MAX_PATH -> 10240
    TCHAR m_strText[10240];      // Window text  
	//@} Jaewon
};
...
// EditBox control
//-----------------------------------------------------------------------------
#define UNISCRIBE_DLLNAME TEXT("\\usp10.dll")
...
public:
    ...
    void SetText( TCHAR* wszText, bool bSelected = false );
    const TCHAR* GetText() { return m_Buffer.GetBuffer(); }
...
public:
    ...
    public:
        ...
        bool InsertChar( int nIndex, TCHAR wChar );
        ...
        // If nIndex == -1, insert to the end.
        bool InsertString( int nIndex, const TCHAR *pStr, int nCount = -1 );

        bool SetText( TCHAR* wszText );
...
//-----------------------------------------------------------------------------
#define IMM32_DLLNAME TEXT("\\imm32.dll")
#define VER_DLLNAME TEXT("\\version.dll")
...
protected:
    ...
    static UINT WINAPI Dummy_GetReadingString( HIMC, UINT, TCHAR*, PINT, BOOL*, PUINT ) { return 0; }
    ...
    static UINT (WINAPI * _GetReadingString)( HIMC, UINT, TCHAR*, PINT, BOOL*, PUINT );
...
protected:
    ...
    static TCHAR*  s_wszCurrIndicator;    // Points to an indicator string that corresponds to current input locale
