...
BOOL XportTool::exportItemTemplate()
{
	...
	{
		PrintConsole("m_cExportResource.ExportItemTemplates() failed!");
		return FALSE;
	}
...
int APIENTRY _tWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	for(int i=1; i<argc; ++i)
	{
		...
		if( !bExport )
		{
			...
			else if( !strcmp( szArgc, "western" ) )		g_eSelectedServiceArea = AP_SERVICE_AREA_GLOBAL;
	...
	case AP_SERVICE_AREA_GLOBAL:	cXportTool.PrintLog( "WESTERN\n" );	break;
