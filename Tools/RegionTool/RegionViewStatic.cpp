...
void CRegionViewStatic::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	else
	{
		...
		if( pParent && pParent->m_pRegionMenu )
		{
			int nIndex = pParent->m_pRegionMenu->m_nCurrentMenu;
			if( nIndex >= 0 && nIndex < ( int )pParent->m_pRegionMenu->m_vectorMenuWnd.size() )
			{
				if( pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu )
				{
					pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu->OnLButtonDownMap( pointOriginal , point .x , point.y );
				}
			}
		}
	}
...
void CRegionViewStatic::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	if( pParent && pParent->m_pRegionMenu )
	{
		...
		if( nIndex >= 0 && nIndex < ( int )pParent->m_pRegionMenu->m_vectorMenuWnd.size() )
		{
			if( pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu )
			{
				pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu->OnLButtonUpMap( pointOriginal , point .x , point.y );
			}
		}
	}
...
void CRegionViewStatic::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if( pParent && pParent->m_pRegionMenu )
	{
		...
		if( nIndex >= 0 && nIndex < ( int )pParent->m_pRegionMenu->m_vectorMenuWnd.size() )
		{
			if( pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu )
			{
				pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu->OnMouseMoveMap( pointOriginal , point .x , point.y );
			}
		}
	}
...
//							else
	...
	if( pParent && pParent->m_pRegionMenu )
	{
		...
		if( nIndex >= 0 && nIndex < ( int )pParent->m_pRegionMenu->m_vectorMenuWnd.size() )
		{
			if( pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu )
			{
				pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu->OnPaintOnMap( & m_memDC );
			}
		}
	}
...
void CRegionViewStatic::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	if( pParent && pParent->m_pRegionMenu )
	{
		...
		if( nIndex >= 0 && nIndex < ( int )pParent->m_pRegionMenu->m_vectorMenuWnd.size() )
		{
			if( pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu )
			{
				pParent->m_pRegionMenu->m_vectorMenuWnd[ nIndex ].pMenu->OnLButtonDblDownMap( pointOriginal , point .x , point.y );
			}
		}
	}
