...
BOOL CModelToolApp::LoadItemTemplate()
{
	...
	case TSO::LE_NO_SEPARATED_TEMPLATE:
		{
			CHAR szErrorCode[AMT_MAX_STR] = { 0, };

			INT	nResult	=	::MessageBox( NULL , TEXT("아이템 템플릿을 통파일로 로딩 하시겠습니까?") , TEXT("Item Read Type ") , MB_YESNO | MB_TOPMOST );


			if( nResult == IDYES )
			{
				if( !m_csAgcEngine.GetAgpmItemModule()->StreamReadTemplate( "INI\\ItemTemplateAll.ini" , NULL , m_bExport) )
				{
					ReleaseStringDlg();
			...
			{
				// 템플릿 파일 읽기 시도
				//if( !m_csAgcEngine.GetAgpmItemModule()->LoadItemTemplateEntryINI( "INI\\ItemTemplateEntry.ini", m_bExport ) )
				if( !m_csAgcEngine.GetAgpmItemModule()->StreamReadTemplates( "INI\\ItemTemplate", "INI\\ItemTemplateEntry.ini", szErrorCode, m_bExport ) )
				{
					ReleaseStringDlg();
					CModelToolDlg::GetInstance()->MessageBox(szErrorCode, "ERROR", MB_ICONERROR | MB_OK);
					CModelToolDlg::GetInstance()->MessageBox("아이템 템플릿 읽기 실패!", "ERROR", MB_ICONERROR | MB_OK);
	...
	for( ; Iter != m_csAgcEngine.GetAgpmItemModule()->csTemplateAdmin.end() ; ++Iter )
	{
		...
		AgcdItemTemplate*	pcsAgcdItemTemplate	=	m_csAgcEngine.GetAgcmItemModule()->GetTemplateData( pTemplate );
		HTREEITEM			hParent				=	CModelToolDlg::GetInstance()->GetResourceForm()->GetTreeHandleItemParent( pTemplate , pcsAgcdItemTemplate );

		if( hParent )
		{
			sprintf( szNewItemName , "%s_%05d" , pTemplate->m_szName , pTemplate->m_lID );
			HTREEITEM hInsertedItem = pcsTreeItem->_InsertItem( szNewItemName , hParent , (DWORD)(pTemplate->m_lID) );
			if( hInsertedItem )
				pcsTreeItem->SetTreeDNF_Flag( hInsertedItem , m_uNationalCode , pcsAgcdItemTemplate->m_nDNF );

			sprintf( pMsgBuff , "%s\n%s" , AMT_START_UP_ITEM_TEMPLATE_STR , pTemplate->m_szName );
		}
...
BOOL CModelToolApp::InitializeModule(CHAR *szDebug)
{
	...
	{
		m_csAgcEngine.GetAgcmEff2Module()->SetGlobalVariable( NULL, NULL, GetRenderWare()->GetWorld(), GetRenderWare()->GetCamera());
		if (!m_bExport)
	...
	{
		m_csAgcEngine.GetAgcmSound()->UnMuteSound( SoundType_Effect );
		m_csAgcEngine.GetAgcmSound()->SetReadFromPack( FALSE );
	}
...
BOOL CModelToolApp::ReloadItemTemplate(BOOL bCurSave)
{
	if(bCurSave)
	{
		if( !m_csAgcEngine.GetAgpmItemModule()->StreamWriteTemplates( "INI\\ItemTemplate", "INI\\ItemTemplateEntry.ini", m_bExport ) ) return FALSE;
		//if( !m_csAgcEngine.GetAgpmItemModule()->StreamWriteTemplate( "INI\\ItemTemplateAll.ini", m_bExport ) ) return FALSE;
