...
BOOL ForEachTemplate< T >::_DoForEach( T * pcsTemplate )
{
	...
	if( AgcEngineChild::SF_NOT_CHANGED != pSaveData->GetState() )
	{
		// COLUMN_TYPE
...
void ForEachItemTemplate::UpdateList( void )
{
	CModelToolApp *pApp = CModelToolApp::GetInstance();
	if( !pApp ) return;

	int nSavedItemTemplateCount = pApp->GetSavedItemTemplateCount();
	for( int nCount = 0 ; nCount < nSavedItemTemplateCount ; nCount++ )
	{
		AgpdItemTemplate* ppdItemTemplate = pApp->GetSavedItemTemplateByIndex( nCount );
		if( ppdItemTemplate )
		{
			AgcEngineChild::stSaveDataInfo* pSaveData = m_csModuleManager.GetAttachedData( ppdItemTemplate );
			if( pSaveData )
			{
				// COLUMN_TYPE
				_csCtrl.InsertItem( _nSequence , GetTypeName() , GetTypeColor() , RGB( 255 , 255 ,255 ) );
				...
				// COLUMN_TID
				CString	str;
				str.Format( "%06d" , ppdItemTemplate->GetID() );
				_csCtrl.SetItemText( _nSequence , CSaveDlg::COLUMN_TID , str );

				// COLUMN_NAME
				_csCtrl.SetItemText( _nSequence , CSaveDlg::COLUMN_NAME , ppdItemTemplate->GetName() );
				...
				// 포인터를 기록해둠.
				_csCtrl.SetItemData( _nSequence , PtrToInt( ppdItemTemplate ) );
				_nSequence++;
			}
		}
	}
