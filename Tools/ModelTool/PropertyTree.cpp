...
VOID CPropertyTree::CreateTree()
{
	...
	m_hParentMaterial			= _InsertItem(PT_PARENT_ITEM_NAME_MATERIAL,			TVI_ROOT,				E_PT_ITEM_DATA_MATERIAL );
	...
	m_hMaterialEdit				= _InsertItem(PT_ITEM_NAME_MATERIAL_EDIT,			m_hParentMaterial,		E_PT_ITEM_DATA_MATERIAL_EDIT);
	m_hMaterialDelete			= _InsertItem(PT_ITEM_NAME_MATERIAL_DELETE,			m_hParentMaterial,		E_PT_ITEM_DATA_MATERIAL_DELETE);
	...
	Expand(m_hParentMaterial, TVE_EXPAND);		SetItemBold(m_hParentMaterial, TRUE);
}
...
void CPropertyTree::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	case E_PT_ITEM_DATA_MATERIAL_EDIT:		bFuncResult	= CModelToolApp::GetInstance()->F_MaterialEdit();		break;
	case E_PT_ITEM_DATA_MATERIAL_DELETE:	bFuncResult	= CModelToolApp::GetInstance()->F_MaterialDelete();		break;
