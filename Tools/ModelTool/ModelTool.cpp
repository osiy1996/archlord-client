...
bool CModelToolApp::CheckFrame()
{
	DWORD CurTick = GetTickCount();

	if( m_Fps.FrameStep < 0 )
	{
		++m_Fps.Count;

		if( CurTick - m_Fps.PrevTick >= 1000 )
		{
			m_Fps.PrevTick = CurTick;
			m_Fps.Fps      = m_Fps.Count;
			m_Fps.Count    = 0;
		}
	}

	else if( CurTick < m_Fps.NextTick )
	{
		return false;
	}
	...
	{
		m_Fps.NextTick  += m_Fps.FrameStep;
		++m_Fps.Count;
...
void CModelToolApp::SetFps( DWORD Fps )
{
	m_Fps.FrameStep = 1000 / Fps;
	m_Fps.NextTick  = GetTickCount() + m_Fps.FrameStep;
}
...
VOID CModelToolApp::EnableSound(BOOL bValue)
{
	...
	{
		bValue ? m_csAgcEngine.GetAgcmSound()->UnMuteSound( SoundType_Effect ) : m_csAgcEngine.GetAgcmSound()->MuteSound( SoundType_Effect );
	}
}

void CModelToolApp::ClearSavedItemTemplate( void )
{
	m_mapItemTemplateSaved.Clear();
}

void CModelToolApp::AddSavedItemTemplate( AgpdItemTemplate* ppdItemTemplate )
{
	if( !ppdItemTemplate ) return;

	stItemTemplateSavedEntry* pEntry = m_mapItemTemplateSaved.Get( ppdItemTemplate->m_lID );
	if( pEntry ) return;

	stItemTemplateSavedEntry NewEntry;

	NewEntry.m_nTID = ppdItemTemplate->m_lID;
	NewEntry.m_ppdItemTemplate = ppdItemTemplate;

	m_mapItemTemplateSaved.Add( ppdItemTemplate->m_lID, NewEntry );

#ifdef _DEBUG
	char strDebug[ 256 ] = { 0, };
	sprintf_s( strDebug, 256, "Save Modified ItemTemplate Entry, TID = %d, Name = %s\n", ppdItemTemplate->m_lID, ppdItemTemplate->m_szName );
	OutputDebugString( strDebug );
#endif
}

void CModelToolApp::DeleteSavedItemTemplate( int nTID )
{
	m_mapItemTemplateSaved.Delete( nTID );
}

int CModelToolApp::GetSavedItemTemplateCount( void )
{
	return m_mapItemTemplateSaved.GetSize();
}

AgpdItemTemplate* CModelToolApp::GetSavedItemTemplate( int nTID )
{
	stItemTemplateSavedEntry* pEntry = m_mapItemTemplateSaved.Get( nTID );
	if( !pEntry ) return NULL;
	return pEntry->m_ppdItemTemplate;
}

AgpdItemTemplate* CModelToolApp::GetSavedItemTemplateByIndex( int nIndex )
{
	stItemTemplateSavedEntry* pEntry = m_mapItemTemplateSaved.GetByIndex( nIndex );
	if( !pEntry ) return NULL;
	return pEntry->m_ppdItemTemplate;
}

BOOL				CModelToolApp::ReleaseAtomicFX( RpAtomic* pAtomic )
{
	m_csAgcEngine.GetAgcmRenderModule()->AtomicFxOff( pAtomic );
