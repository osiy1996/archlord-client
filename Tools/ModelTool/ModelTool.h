...
public:
	...
	BOOL					ReleaseAtomicFX( RpAtomic*	pAtomic );
...
public:
	...
	void	SetSaveDataItem()
	{
		...
		AddSavedItemTemplate( ppTemplate );
	}

	void	SetSaveDataItem( AgpdItemTemplate*	pTemplate )
	{
		if( !pTemplate )
			return;

		m_csAgcEngine.SetSaveData( pTemplate );
		AddSavedItemTemplate( pTemplate );
	}
...
public :
	struct stItemTemplateSavedEntry
	{
		int									m_nTID;
		AgpdItemTemplate*					m_ppdItemTemplate;

		stItemTemplateSavedEntry( void )
		{
			m_nTID = 0;
			m_ppdItemTemplate = NULL;
		}
	};
	
	ContainerMap< int, stItemTemplateSavedEntry >		m_mapItemTemplateSaved;

public :
	void				ClearSavedItemTemplate			( void );
	void				AddSavedItemTemplate			( AgpdItemTemplate* ppdItemTemplate );
	void				DeleteSavedItemTemplate			( int nTID );
	int					GetSavedItemTemplateCount		( void );
	AgpdItemTemplate*	GetSavedItemTemplate			( int nTID );
	AgpdItemTemplate*	GetSavedItemTemplateByIndex		( int nIndex );
};