...
{
	m_csAgcEngine.GetAgcmEff2()->ForTool_ReloadAllEffect();
	return TRUE;
}
...
BOOL CModelToolApp::F_MaterialEdit()
{
	if( !(m_eObjectType & AMT_OBJECT_TYPE_CHARACTER) && !(m_eObjectType & AMT_OBJECT_TYPE_ITEM) )
		return FALSE;

	CModelToolDlg::GetInstance()->OpenMaterialEditDlg();
...
BOOL CModelToolApp::F_MaterialDelete()
{	
	if( !m_eObjectType & AMT_OBJECT_TYPE_CHARACTER && !m_eObjectType & AMT_OBJECT_TYPE_ITEM )
		return FALSE;

	CModelToolDlg::GetInstance()->DeleteMaterial();
