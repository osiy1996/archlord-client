...
void CRenderWare::EndScene( BOOL bvSync )
{
	if( RwCameraEndUpdate(m_Camera) )
	{
		if( !bvSync )
			RwCameraShowRaster( m_Camera, m_hWnd, rwRASTERFLIPDONTWAIT );
		else
			RwCameraShowRaster( m_Camera, m_hWnd, rwRASTERFLIPWAITVSYNC );
	}
}
...
VOID CRenderWare::ShowRWSInfo( RpClump*	pClump )
{
	if( !m_Charset || !pClump )
		return;
	...
	std::list< RwTexture* >		listTexture;

	ZeroMemory( parrAtomic , sizeof(parrAtomic) );
	RpClumpForAllAtomics( pClump , CBAllAtomic , parrAtomic );

	RwUtilPrintText( m_Charset, "-----Using Texture Info-----", 0, nIndex++, rsPRINTPOSTOPLEFT );
	...
	for( INT k = 0 ; parrAtomic[k] ; ++k )
	{
		...
		{
			CHAR*		szTexName	=	NULL;
			RwTexture*	pTexture	=	RpMaterialGetTexture( ppMaterial[i] );
			if( pTexture )
			{
				szTexName	=	RwTextureGetName( pTexture );
				
			}

			if( szTexName )
			{
				if( std::find( listTexture.begin() , listTexture.end() , pTexture ) == listTexture.end() )
				{
					RwUtilPrintText( m_Charset, szTexName, 0, nIndex++, rsPRINTPOSTOPLEFT );
					listTexture.push_back( pTexture );
				}
			}
		}
	}
...
RpAtomic*	CRenderWare::CBAllAtomic( RpAtomic* pAtomic , PVOID pData )
{
	if( !pAtomic )
		return NULL;
