...
VOID CModelToolDlg::OpenMaterialEditDlg()
{
	if( !m_pcsMaterialEditDlg )
	{
		m_pcsMaterialEditDlg	=	new	CMaterialEditDlg;
		m_pcsMaterialEditDlg->Create( IDD_MATERIAL_EDIT , this );
	}

	RwUtilClumpArray*	pClumpArray		=	CModelToolApp::GetInstance()->GetRenderClump();
	if( pClumpArray )
	{
		m_pcsMaterialEditDlg->SetClump( pClumpArray->array[0] );
		m_pcsMaterialEditDlg->ShowWindow( SW_SHOW );
	}
}

VOID CModelToolDlg::CloseMaterialEditDlg()
{
	if( m_pcsMaterialEditDlg )
	{
		m_pcsMaterialEditDlg->ShowWindow( SW_HIDE );
	}
}

VOID CModelToolDlg::DeleteMaterial()
{
	if( m_pcsMaterialEditDlg )
	{
	}
