...
BOOL CWebzenShop::InquireCash(DWORD GameCode, int ServerIndex, char *AccountID)
{
	return g_LibClientSessionShop->InquireCash(GameCode, ServerIndex, AccountID);
}
...
BOOL CWebzenShop::BuyProduct(DWORD GameCode, int ServerIndex, char *AccountID, CHAR *charName, double DefuctPrice, INT64 ItemNo, INT64 OrderNo, int OrderQuantity, DWORD AccessIP)
{
	USES_CONVERSION;

	WCHAR *wCharName	= A2W( charName );

	return g_LibClientSessionShop->BuyProduct(GameCode, ServerIndex, AccountID, wCharName, DefuctPrice, ItemNo, OrderNo, OrderQuantity, AccessIP);
}
