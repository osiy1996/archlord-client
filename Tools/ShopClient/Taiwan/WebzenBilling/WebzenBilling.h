...
typedef struct
{
	...
	stTWInquire()
		: lResultCode(0), lCoinSum(0)
	{
		ZeroMemory(AccountID, sizeof(AccountID));
	}
} stTWInquire;
...
public:
	...
	// AR_TW Start 
	BOOL InquireCash(DWORD GameCode, int ServerIndex, char *AccountID);
	BOOL BuyProduct(DWORD GameCode, int ServerIndex, char* AccountID, CHAR *charName, double DefuctPrice,
					INT64 ItemNo, INT64 OrderNo, int OrderQuantity, DWORD AccessIP);
