...
public:
	...
	// AR_TW Start : 캐시 및 포인트를 조회한다.
	bool InquireCash(
		DWORD	GameCode,						// 게임코드
		int		ServerIndex,					// 서버번호
		char	AccountID[MAX_ACCOUNT_LENGTH]	// ID
		);
	// AR_TW End

	// AR_TW Start : 물품을 구매한다.
	bool BuyProduct(
		DWORD	GameCode,							// 게임코드
		int		ServerIndex,						// 서버번호
		char	AccountID[MAX_ACCOUNT_LENGTH],		// ID
		WCHAR	charName[MAX_CHARNAME_TW_LENGTH],	// 캐릭터이름
		double	DefuctPrice,						// 차감캐시
		INT64	ItemNo,								// 아이템번호
		INT64	OrderNo,							// 주문번호
		int		OrderQuantity,						// 주문수량
		DWORD	AccessIP							// IP어드레스
		);
	// AR_TW End
...
protected:
	...
	// AR_TW Start : 캐시 조회의 결과
	virtual void OnInquireCash(char AccountID[MAX_ACCOUNT_LENGTH], 
							   double Cash,
							   long ResultCode) = 0;
	// AR_TW End

	// AR_TW End :  상품 구매의 결과
	virtual void OnBuyProduct(char	AccountID[MAX_ACCOUNT_LENGTH], 
							  INT64	ItemNo,
							  INT64 OrderNo,
							  int	OrderNumber,
