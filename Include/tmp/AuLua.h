...
#elif _WIN32
	#ifdef _DEBUG
	#pragma comment(lib, "lua5.1.2.Win32.static.MDd.lib")
	#else
	#pragma comment(lib, "lua5.1.2.Win32.static.MD.lib")
	#endif
#else
#endif
...
public:
	...
	void		PushNumber64( __int64 nValue );

	void					CallLuaFunction				( const char* pLuaFunctionName );
	void					CallLuaFunction				( const char* pLuaFunctionName, int nParam );
	void					CallLuaFunction				( const char* pLuaFunctionName, char* pParam );
	void					CallLuaFunction				( const char* pLuaFunctionName, char* pParam, int nParam );
	int						CallLuaFunctionInt			( const char* pLuaFunctionName );
	float					CallLuaFunctionFloat		( const char* pLuaFunctionName );
	std::string				CallLuaFunctionString		( const char* pLuaFunctionName );
