// XTExcelTabCtrl.h interface for the CXTExcelTabCtrl class.
//
...
#if !defined(__XTEXCELTABCTRL_H__)
#define __XTEXCELTABCTRL_H__
//}}AFX_CODEJOCK_PRIVATE
...
//     CXTExcelTabCtrlButtonState to determine which arrow is to be
//     displayed for a particular tab button.
// See Also:
//     CXTExcelTabCtrlButtonState, CXTExcelTabCtrl
//
...
//===========================================================================
// Summary:
//     The CXTExcelTabCtrlButtonState class is used by the CXTExcelTabCtrl class
...
//===========================================================================
class _XTP_EXT_CLASS CXTExcelTabCtrlButtonState
{
public:
	//-------------------------------------------------------------------------
	// Summary:
	//     Constructs a CXTExcelTabCtrlButtonState object.
	//-------------------------------------------------------------------------
...
//{{AFX_CODEJOCK_PRIVATE
typedef CArray <CXTTcbItem*, CXTTcbItem*> CTabItemArray;
//}}AFX_CODEJOCK_PRIVATE

//===========================================================================
// Summary:
//     CXTExcelTabCtrl is a CWnd derived class.  It is used to create an Excel
//     style sheet control. This control allows you to define if you want the
//     control to have home, end, back, and next buttons.
//===========================================================================
class _XTP_EXT_CLASS CXTExcelTabCtrl : public CWnd, public CXTThemeManagerStyleHostBase
{
	DECLARE_DYNAMIC(CXTExcelTabCtrl)
	DECLARE_THEME_HOST(CXTExcelTabCtrl)
	DECLARE_THEME_REFRESH(CXTExcelTabCtrl)

public:
	//-----------------------------------------------------------------------
	// Summary:
	//     Constructs a CXTExcelTabCtrl object
	//-----------------------------------------------------------------------
	CXTExcelTabCtrl();

	//-----------------------------------------------------------------------
	// Summary:
	//     Destroys a CXTExcelTabCtrl object, handles cleanup and deallocation
	//-----------------------------------------------------------------------
	virtual ~CXTExcelTabCtrl();
...
public:
	...
	virtual int InsertItem(int nItem, LPCTSTR lpszItem, CWnd* pWndControl = NULL); //<combine CXTExcelTabCtrl::InsertItem@int@UINT@CWnd* >
	...
	void SetTabBackColor(int nIndex, COLORREF crBack); // <combine CXTExcelTabCtrl::SetTabBackColor@COLORREF>
	...
	COLORREF GetTabBackColor(int nIndex) const; // <combine CXTExcelTabCtrl::GetTabBackColor@const>
	...
	void SetTabTextColor(int nIndex, COLORREF crText); // <combine CXTExcelTabCtrl::SetTabTextColor@COLORREF>
	...
	COLORREF GetTabTextColor(int nIndex) const; // <combine CXTExcelTabCtrl::GetTabTextColor@const>
	...
	void SetSelTabBackColor(int nIndex, COLORREF crBack); // <combine CXTExcelTabCtrl::SetSelTabBackColor@COLORREF>
	...
	COLORREF GetSelTabBackColor(int nIndex) const; // <combine CXTExcelTabCtrl::GetSelTabBackColor@const>
	...
	void SetSelTabTextColor(int nIndex, COLORREF crText); // <combine CXTExcelTabCtrl::SetSelTabTextColor@COLORREF>
	...
	COLORREF GetSelTabTextColor(int nIndex) const; // <combine CXTExcelTabCtrl::GetSelTabTextColor@const>
	...
	//     see the CXTExcelTabCtrl::Create method for a list of
	//     available styles.
	// See Also:
	//     Create, SetTabStyle
	// ---------------------------------------------------------------------
	...
	//     styles, see the CXTExcelTabCtrl::Create method for a list of
	//     available styles.
	// See Also:
	//     Create, GetTabStyle
	// --------------------------------------------------------------------------------
	...
	// ----------------------------------------------------------------------------------------
	// Summary:
	//     Creates a flat tab control and attaches it to an instance of a CXTExcelTabCtrl
	//     object.
	// Parameters:
	...
	//     otherwise FALSE.
	// Remarks:
	//     You construct a CXTExcelTabCtrl object in two steps. First,
	//     call the constructor, and then call Create, which creates the tab
	//     control and attaches it to the CXTExcelTabCtrl object. You
...
protected:
	...
	CXTExcelTabCtrlButtonState  m_buttons[4];    // Array of button information.
	CTabItemArray               m_tcbItems;      // Template list containing tab information.
};

//////////////////////////////////////////////////////////////////////

AFX_INLINE BOOL CXTExcelTabCtrl::Create(LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, const RECT& rect, CWnd* pParentWnd, UINT nID, CCreateContext* pContext) {
	return CWnd::Create(lpszClassName, lpszWindowName, dwStyle, rect, pParentWnd, nID, pContext);
}
AFX_INLINE int CXTExcelTabCtrl::InsertItem(int nItem, UINT nTextID, CWnd* pWndControl) {
	ASSERT(IsWindow(m_hWnd)); CString strItem; strItem.LoadString(nTextID); return InsertItem(nItem, strItem, pWndControl);
}
AFX_INLINE int CXTExcelTabCtrl::GetCurSel() const {
	ASSERT(IsWindow(m_hWnd)); return m_nCurSel;
}
AFX_INLINE int CXTExcelTabCtrl::GetItemCount() const {
	ASSERT(IsWindow(m_hWnd)); int iItemCount = (int)m_tcbItems.GetSize(); return iItemCount;
}
AFX_INLINE void CXTExcelTabCtrl::SetTabFonts(CFont* pNormFont, CFont* pBoldFont) {
	if (pNormFont){ m_pNormFont = pNormFont; } if (pBoldFont){ m_pBoldFont = pBoldFont; }
}
AFX_INLINE DWORD CXTExcelTabCtrl::GetTabStyle() {
	return m_dwStyle;
}
AFX_INLINE  CRect CXTExcelTabCtrl::GetTabsRect() {
	return m_rectTabs;
}
AFX_INLINE CFont* CXTExcelTabCtrl::GetTabFont(BOOL bBold) {
	return bBold ? m_pBoldFont : m_pNormFont;
}

const DWORD FTS_XT_BOTTOM          = 0x0001;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_HASARROWS       = 0x0002;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_HASHOMEEND      = 0x0004;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_TOOLTIPS        = 0x0008;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_DEFAULT         = 0x000F;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_HSCROLL         = 0x0010;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>
const DWORD FTS_XT_MASK            = 0x001F;  //<ALIAS CXTExcelTabCtrl::Create@DWORD@const CRect&@CWnd*@UINT>

//{{AFX_CODEJOCK_PRIVATE
class _XTP_EXT_CLASS CXTFlatTabCtrl : public CXTExcelTabCtrl // deprecated.
{
	DECLARE_DYNAMIC(CXTFlatTabCtrl)
};
//}}AFX_CODEJOCK_PRIVATE

#endif // #if !defined(__XTEXCELTABCTRL_H__)
