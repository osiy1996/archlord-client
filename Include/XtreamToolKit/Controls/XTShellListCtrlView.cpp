...
void CXTShellListView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_RETURN)
	{
	...
	CXTShellListViewBase::OnKeyDown(nChar, nRepCnt, nFlags);
}
...
{
	if (!CXTShellListViewBase::PreCreateWindow(cs))
		return FALSE;
...
void CXTShellListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	if (nChar == VK_RETURN)
	{
		...
		if (iIndex >= 0 && m_pTreeCtrl != NULL)
		{
			m_pTreeCtrl->SendMessage(XTWM_SHELL_NOTIFY,
				(WPARAM)SHN_XT_SELECTCHILD, (LPARAM)(XT_LVITEMDATA*)GetItemData(iIndex));
		}
	}
