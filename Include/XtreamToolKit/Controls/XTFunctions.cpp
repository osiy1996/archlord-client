...
_XTP_EXT_CLASS HBITMAP AFXAPI XTFuncLoadSysColorBitmap(HINSTANCE hInst, HRSRC hRsrc, BOOL bMono)
{
	...
	if (!bConverted)
	{
		...
		const int nTestDibSize = sizeof(BITMAPINFOHEADER) + nTestWidth * 3;
...
_XTP_EXT_CLASS BOOL AFXAPI XTFuncContextMenu(CMenu* pPopup, UINT nFlags, int x, int y, CWnd* pWnd, int nIDBitmap, BOOL bNoNotify)
{
	BOOL bResult = FALSE;
	nIDBitmap;
	bNoNotify;
