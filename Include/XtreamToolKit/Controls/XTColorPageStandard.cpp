...
void CXTColorHex::OnPaint()
{
	...
	if (m_dcPicker.GetSafeHdc() == NULL)
	{
		...
		HBRUSH hBrush = (HBRUSH)GetParent()->SendMessage(WM_CTLCOLORSTATIC, (WPARAM)m_dcPicker.GetSafeHdc(), (LPARAM)m_hWnd);
		if (hBrush)
		{
			::FillRect(m_dcPicker.GetSafeHdc(), rc, hBrush);
		}
		else
		{
			m_dcPicker.FillSolidRect(rc, GetXtremeColor(COLOR_3DFACE));
		}
...
BOOL CXTColorHex::PreCreateWindow(CREATESTRUCT& cs)
{
...
: CPropertyPage(CXTColorPageStandard::IDD)
{
